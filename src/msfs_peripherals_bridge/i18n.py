...
_STRINGS: dict[str, dict[str, str]] = {
    ...
    "(keine gescannten Eingänge)": {
        "en": "(no scanned inputs)",
        "es": "(sin entradas escaneadas)",
        "fr": "(aucune entrée scannée)",
    },
    "+ Anzeige hinzufügen…": {
        "en": "+ Add display…",
        "es": "+ Añadir indicador…",
        "fr": "+ Ajouter un afficheur…",
    },
    ...
    "+ Input anlernen…": {
        "en": "+ Teach input…",
        "es": "+ Aprender entrada…",
        "fr": "+ Apprendre une entrée…",
    },
    "Achse anlernen": {"en": "Teach axis", "es": "Aprender eje", "fr": "Apprendre l'axe"},
    "Achse von Anschlag zu Anschlag bewegen.": {
        "en": "Move the axis from stop to stop.",
        "es": "Mueve el eje de tope a tope.",
        "fr": "Déplacez l'axe de butée à butée.",
    },
    "Achsen-Anlernen geht nur für angesteckte evdev-Geräte (Yoke/Pedale/Quadrant).": {
        "en": "Teaching an axis only works for a connected evdev device (yoke/pedals/quadrant).",
        "es": "Aprender un eje solo funciona con un dispositivo evdev conectado (yugo/pedales/cuadrante).",
        "fr": "L'apprentissage d'un axe ne fonctionne qu'avec un appareil evdev connecté (yoke/palonnier/quadrant).",
    },
    "Adresse": {"en": "Address", "es": "Dirección", "fr": "Adresse"},
    "Aktiv": {"en": "Active", "es": "Activo", "fr": "Actif"},
    "Aktuelles Gerät als Vorlage speichern…": {
        "en": "Save current device as template…",
        "es": "Guardar el dispositivo actual como plantilla…",
        "fr": "Enregistrer l'appareil actuel comme modèle…",
    },
    "Alle angeschlossenen Geräte anzeigen (auch fremde, noch nicht registrierte) und neue Geräte anlegen · Geräte-Paket importieren: Rechtsklick auf ein Gerät": {
        "en": "Show all connected devices (including unknown, not-yet-registered ones) and create new devices · Import a device package: right-click a device",
        "es": "Muestra todos los dispositivos conectados (también los desconocidos, aún no registrados) y crea nuevos dispositivos · Importar un paquete de dispositivo: clic derecho en un dispositivo",
    ...
    "Alle angeschlossenen Geräte — auch noch nicht registrierte. Ein unregistriertes Gerät markieren und „Registrieren…“, damit es im Profil mappbar wird.": {
        "en": "All connected devices — including not-yet-registered ones. Select an unregistered device and “Register…” so it can be mapped in the profile.",
        "es": "Todos los dispositivos conectados, incluidos los aún no registrados. Selecciona un dispositivo no registrado y «Registrar…» para poder mapearlo en el perfil.",
        "fr": "Tous les appareils connectés — y compris ceux non encore enregistrés. Sélectionnez un appareil non enregistré et « Enregistrer… » pour pouvoir le mapper dans le profil.",
    },
    "Alles aus": {"en": "Stop all", "es": "Detener todo", "fr": "Tout arrêter"},
    "Als Geräte-Paket exportieren…": {
        "en": "Export as device package…",
        "es": "Exportar como paquete de dispositivo…",
        "fr": "Exporter comme paquet d'appareil…",
    },
    ...
    "Anzeigen (Schreiben)": {
        "en": "Displays (write)",
        "es": "Indicadores (escritura)",
        "fr": "Afficheurs (écriture)",
    },
    "Art": {"en": "Type", "es": "Tipo", "fr": "Type"},
    "Auf gewähltes Gerät anlegen": {
        "en": "Create on selected device",
        "es": "Crear en el dispositivo seleccionado",
        "fr": "Créer sur l'appareil sélectionné",
    },
    "Aus Vorlage füllen": {
        "en": "Fill from template",
        "es": "Rellenar desde plantilla",
        "fr": "Remplir depuis un modèle",
    },
    "Aus Vorlage füllen…": {
        "en": "Fill from template…",
        "es": "Rellenar desde plantilla…",
    ...
    "Aus dem aktuellen Mapping übernehmen:": {
        "en": "Take from the current mapping:",
        "es": "Tomar del mapeo actual:",
        "fr": "Reprendre du mappage actuel :",
    },
    "Aus der Geräteliste entfernen…": {
        "en": "Remove from device list…",
        "es": "Quitar de la lista de dispositivos…",
        "fr": "Retirer de la liste des appareils…",
    },
    "Ausgabe hinzugefügt ✓": {
        "en": "Output added ✓",
        "es": "Salida añadida ✓",
    ...
    "Ausgang-Scan geht nur für angesteckte hidraw-Panels.": {
        "en": "Output scan only works for a connected hidraw panel.",
        "es": "El escaneo de salidas solo funciona con un panel hidraw conectado.",
        "fr": "Le scan des sorties ne fonctionne qu'avec un panneau hidraw connecté.",
    },
    "Ausgeblendete Geräte": {
        "en": "Hidden devices",
        "es": "Dispositivos ocultos",
    ...
    "Bedingung 2 (optional)": {
        "en": "Condition 2 (optional)",
        "es": "Condición 2 (opcional)",
        "fr": "Condition 2 (facultatif)",
    },
    "Beim Ausschalten — leer = nichts": {
        "en": "On switch-off — empty = nothing",
        "es": "Al apagar — vacío = nada",
        "fr": "À l'extinction — vide = rien",
    },
    "Beim Einschalten": {"en": "On switch-on", "es": "Al encender", "fr": "À l'allumage"},
    "Bit (0-7)": {"en": "Bit (0-7)", "es": "Bit (0-7)", "fr": "Bit (0-7)"},
    "Bridge starten und, sobald Port 7842 offen ist (MSFS mit Flug), den Mapper. Erst MSFS + Flug laden.": {
        "en": "Start the bridge and, once port 7842 is open (MSFS with a flight), the mapper. Load MSFS + a flight first.",
        "es": "Inicia el bridge y, en cuanto el puerto 7842 esté abierto (MSFS con un vuelo), el mapeador. Carga primero MSFS + un vuelo.",
        "fr": "Démarre le bridge puis, dès que le port 7842 est ouvert (MSFS avec un vol), le mappeur. Chargez d'abord MSFS + un vol.",
    },
    "Byte/Bit (bzw. Zelle) nicht bekannt? Fast nie nötig, von Hand einzutippen: „{btn}“ schickt Testimpulse ans Panel — leuchtet DEINE LED/Zelle, „Das ist es!“ klicken; die Felder füllen sich automatisch (Panel angesteckt, Mapper gestoppt).": {
        "en": "Byte/bit (or cell) unknown? Almost never needs typing by hand: “{btn}” sends test pulses to the panel — when YOUR LED/cell lights up, click “That's it!”; the fields fill in automatically (panel plugged in, mapper stopped).",
        "es": "¿No conoces el byte/bit (o la celda)? Casi nunca hace falta escribirlo a mano: «{btn}» envía impulsos de prueba al panel — cuando se encienda TU LED/celda, pulsa «¡Es este!»; los campos se rellenan solos (panel conectado, mapeador detenido).",
        "fr": "Byte/bit (ou cellule) inconnu ? Presque jamais besoin de le saisir à la main : « {btn} » envoie des impulsions de test au panneau — quand VOTRE LED/cellule s'allume, cliquez sur « C'est ça ! » ; les champs se remplissent automatiquement (panneau branché, mappeur arrêté).",
    },
    "Code": {"en": "Code", "es": "Código", "fr": "Code"},
    "Das ist es!": {"en": "That's it!", "es": "¡Es este!", "fr": "C'est ça !"},
    "Detail": {"en": "Detail", "es": "Detalle", "fr": "Détail"},
    "Die „Adresse“ der LED im Gerät. Das Gerät bekommt seine Befehle als eine Reihe nummerierter Felder (Bytes); das Byte sagt, in WELCHEM Feld diese LED sitzt. Musst du nicht wissen — „🔦 Byte/Bit suchen…“ füllt es aus.": {
        "en": "The LED's “address” in the device. The device receives its commands as a series of numbered fields (bytes); the byte says in WHICH field this LED sits. You don't need to know it — “🔦 Find byte/bit…” fills it in.",
        "es": "La «dirección» del LED en el dispositivo. El dispositivo recibe sus órdenes como una serie de campos numerados (bytes); el byte indica en QUÉ campo está este LED. No necesitas saberlo: «🔦 Buscar byte/bit…» lo rellena.",
        "fr": "L'« adresse » de la LED dans l'appareil. L'appareil reçoit ses commandes sous forme d'une série de champs numérotés (octets) ; l'octet indique dans QUEL champ se trouve cette LED. Pas besoin de le savoir — « 🔦 Trouver octet/bit… » le remplit.",
    },
    "Die „Adresse“ der ersten Ziffer im Gerät (das nummerierte Feld, in dem die Anzeige beginnt). Musst du nicht wissen — „🔦 Zelle suchen…“ füllt es aus.": {
        "en": "The “address” of the first digit in the device (the numbered field where the display starts). You don't need to know it — “🔦 Find cell…” fills it in.",
        "es": "La «dirección» del primer dígito en el dispositivo (el campo numerado donde empieza el indicador). No necesitas saberlo: «🔦 Buscar celda…» lo rellena.",
        "fr": "L'« adresse » du premier chiffre dans l'appareil (le champ numéroté où commence l'afficheur). Pas besoin de le savoir — « 🔦 Trouver cellule… » le remplit.",
    },
    "Diese Geräte hast du aus der Geräteliste entfernt. Auswählen und „Wieder anzeigen“, um sie zurückzuholen.": {
        "en": "These devices you removed from the device list. Select one and “Show again” to bring it back.",
        "es": "Estos dispositivos los has quitado de la lista. Selecciona uno y «Mostrar de nuevo» para recuperarlo.",
        "fr": "Ces appareils, vous les avez retirés de la liste. Sélectionnez-en un et « Réafficher » pour le récupérer.",
    },
    "Dieses Gerät hat im aktuellen Profil keine Mappings.": {
        "en": "This device has no mappings in the current profile.",
        "es": "Este dispositivo no tiene mapeos en el perfil actual.",
        "fr": "Cet appareil n'a aucun mappage dans le profil actuel.",
    },
    "Dieses Gerät hat keine Anzeige/Ausgabe zum Speichern.": {
        "en": "This device has no display/output to save.",
        "es": "Este dispositivo no tiene indicador/salida para guardar.",
        "fr": "Cet appareil n'a aucun afficheur/sortie à enregistrer.",
    },
    "Dieses Gerät ist bereits registriert.": {
        "en": "This device is already registered.",
        "es": "Este dispositivo ya está registrado.",
        "fr": "Cet appareil est déjà enregistré.",
    },
    "Dieses Gerät ist nicht registriert.": {
        "en": "This device is not registered.",
        "es": "Este dispositivo no está registrado.",
        "fr": "Cet appareil n'est pas enregistré.",
    },
    "Display": {"en": "Display", "es": "Pantalla", "fr": "Afficheur"},
    "Display (7-Segment)": {
        "en": "Display (7-segment)",
        "es": "Pantalla (7 segmentos)",
        "fr": "Afficheur (7 segments)",
    },
    "Display (7-Segment)…": {
        "en": "Display (7-segment)…",
        "es": "Pantalla (7 segmentos)…",
    ...
    "Drücke den Knopf mehrmals.": {
        "en": "Press the button several times.",
        "es": "Pulsa el botón varias veces.",
        "fr": "Appuyez plusieurs fois sur le bouton.",
    },
    "Eigene Vorlagen:": {"en": "Your templates:", "es": "Tus plantillas:", "fr": "Vos modèles :"},
    "Ein Element aufleuchten lassen, um es am echten Panel zu identifizieren. Geht nur, wenn der Mapper das Panel NICHT steuert (er würde den Test sofort überschreiben).": {
        "en": "Light up an element to identify it on the real panel. Only works when the mapper is NOT driving the panel (it would overwrite the test immediately).",
        "es": "Encender un elemento para identificarlo en el panel real. Solo funciona si el mapeador NO controla el panel (sobrescribiría la prueba al instante).",
        "fr": "Faire clignoter un élément pour l'identifier sur le vrai panneau. Ne fonctionne que si le mappeur ne pilote PAS le panneau (il écraserait le test aussitôt).",
    },
    "Ein Testimpuls wandert über die Report-Adressen. Sobald am Gerät die richtige LED/Ziffer aufleuchtet: „Das ist es!“. Nur wenn der Mapper NICHT läuft.": {
        "en": "A test pulse walks across the report addresses. As soon as the right LED/digit lights up on the device: “That's it!”. Only when the mapper is NOT running.",
        "es": "Un impulso de prueba recorre las direcciones del reporte. En cuanto se encienda el LED/dígito correcto en el dispositivo: «¡Es este!». Solo si el mapeador NO está en marcha.",
        "fr": "Une impulsion de test parcourt les adresses du rapport. Dès que la bonne LED/le bon chiffre s'allume sur l'appareil : « C'est ça ! ». Uniquement si le mappeur ne tourne PAS.",
    },
    "Ein vorhandenes SPAD.neXt-Profil (.xml) einlesen: die Events/SimVars, die dieses Flugzeug nutzt, erscheinen beim Mappen ganz oben im Variablen-Picker (kuratierte Kurzliste statt 700+ Namen). Nur die Semantik — den physischen Knopf wählst du weiter selbst.": {
        "en": "Read an existing SPAD.neXt profile (.xml): the events/SimVars this aircraft uses appear at the top of the variable picker when mapping (a curated short list instead of 700+ names). Semantics only — you still pick the physical button yourself.",
        "es": "Lee un perfil SPAD.neXt existente (.xml): los eventos/SimVars que usa esta aeronave aparecen arriba en el selector de variables al mapear (una lista breve curada en vez de más de 700 nombres). Solo la semántica: el botón físico lo sigues eligiendo tú.",
        "fr": "Lire un profil SPAD.neXt existant (.xml) : les événements/SimVars utilisés par cet avion apparaissent en haut du sélecteur de variables lors du mappage (une courte liste triée plutôt que plus de 700 noms). La sémantique seulement — le bouton physique, c'est toujours vous qui le choisissez.",
    },
    "Eine Anzeige des Geräts an eine Variable binden — LED (ein Bit) oder Display (7-Segment-Wert ← Var). Legt bei Bedarf den generischen Ausgabe-Block an (Schritt E).": {
        "en": "Bind a device display to a variable — LED (one bit) or display (7-segment value ← var). Creates the generic output block if needed (step E).",
        "es": "Vincula un indicador del dispositivo a una variable — LED (un bit) o pantalla (valor de 7 segmentos ← var). Crea el bloque de salida genérico si hace falta (paso E).",
        "fr": "Lier un afficheur de l'appareil à une variable — LED (un bit) ou afficheur (valeur 7 segments ← var). Crée le bloc de sortie générique si nécessaire (étape E).",
    },
    "Eine einzelne Eingabe an ein Event/eine Variable binden — Taster/Schalter/Achse/Hat. Ein Encoder (2 Richtungen) wird an generischen Geräten in einem Rutsch angelernt; die Saitek-Panels steuern ihre Encoder über die Vorlage (kein eigener Schritt).": {
        "en": "Bind a single input to an event/variable — button/switch/axis/hat. An encoder (2 directions) is taught in one go on generic devices; the Saitek panels drive their encoders via the template (no separate step).",
        "es": "Vincula una sola entrada a un evento/variable — botón/interruptor/eje/hat. Un encoder (2 direcciones) se aprende de una vez en dispositivos genéricos; los paneles Saitek controlan sus encoders mediante la plantilla (sin paso aparte).",
        "fr": "Lier une seule entrée à un événement/une variable — bouton/interrupteur/axe/hat. Un encodeur (2 directions) s'apprend d'un coup sur les appareils génériques ; les panneaux Saitek pilotent leurs encodeurs via le modèle (pas d'étape séparée).",
    },
    "Einen bereits gescannten, benannten Eingang des Geräts wählen (Geräte-Explorer, Eingänge scannen) — füllt Art und Code automatisch.": {
        "en": "Pick an already scanned, named input of the device (Device Explorer, scan inputs) — fills type and code automatically.",
        "es": "Elige una entrada ya escaneada y con nombre del dispositivo (Explorador de dispositivos, escanear entradas) — rellena tipo y código automáticamente.",
        "fr": "Choisir une entrée déjà scannée et nommée de l'appareil (Explorateur d'appareils, scanner les entrées) — remplit le type et le code automatiquement.",
    },
    "Eingänge scannen": {
        "en": "Scan inputs",
        "es": "Escanear entradas",
        "fr": "Scanner les entrées",
    },
    "Elemente aus dem aktuellen Mapping übernehmen (Bindings + Panel-Controls/LEDs/Display)": {
        "en": "Take elements from the current mapping (bindings + panel controls/LEDs/display)",
        "es": "Tomar elementos del mapeo actual (bindings + controles del panel/LEDs/pantalla)",
        "fr": "Reprendre des éléments du mappage actuel (bindings + commandes du panneau/LED/afficheur)",
    },
    "Elemente frei anordnen: im Nachbau die Knöpfe/Anzeigen ins Raster ziehen, an der blauen Ecke in der Größe ziehen (Rechtsklick = Größe in Pixel). Pro Gerät gespeichert.": {
        "en": "Arrange elements freely: in the replica drag the buttons/displays onto the grid, drag the blue corner to resize (right-click = size in pixels). Saved per device.",
        "es": "Organiza los elementos libremente: en la réplica arrastra los botones/indicadores a la cuadrícula, arrastra la esquina azul para cambiar el tamaño (clic derecho = tamaño en píxeles). Se guarda por dispositivo.",
    ...
    "Encoder (2 Richtungen)…": {
        "en": "Encoder (2 directions)…",
        "es": "Encoder (2 direcciones)…",
        "fr": "Encodeur (2 directions)…",
    },
    "Encoder anlernen": {
        "en": "Teach encoder",
        "es": "Aprender encoder",
        "fr": "Apprendre l'encodeur",
    },
    "Encoder gegen den Uhrzeigersinn drehen.": {
        "en": "Turn the encoder counter-clockwise.",
        "es": "Gira el encoder en sentido antihorario.",
        "fr": "Tournez l'encodeur dans le sens antihoraire.",
    },
    "Encoder im Uhrzeigersinn drehen.": {
        "en": "Turn the encoder clockwise.",
        "es": "Gira el encoder en sentido horario.",
        "fr": "Tournez l'encodeur dans le sens horaire.",
    },
    "Encoder mehrmals GEGEN den Uhrzeigersinn drehen.": {
        "en": "Turn the encoder COUNTER-clockwise several times.",
        "es": "Gira el encoder varias veces en sentido ANTIHORARIO.",
        "fr": "Tournez l'encodeur plusieurs fois dans le sens ANTIHORAIRE.",
    },
    "Encoder mehrmals IM Uhrzeigersinn drehen.": {
        "en": "Turn the encoder CLOCKWISE several times.",
        "es": "Gira el encoder varias veces en sentido HORARIO.",
        "fr": "Tournez l'encodeur plusieurs fois dans le sens HORAIRE.",
    },
    "Encoder-Anlernen abgebrochen (nicht beide Richtungen).": {
        "en": "Encoder teaching cancelled (not both directions).",
        "es": "Aprendizaje del encoder cancelado (no ambas direcciones).",
        "fr": "Apprentissage de l'encodeur annulé (pas les deux directions).",
    },
    "Encoder-Name": {"en": "Encoder name", "es": "Nombre del encoder", "fr": "Nom de l'encodeur"},
    "Erfasst": {"en": "Captured", "es": "Capturado", "fr": "Capturé"},
    "Erst registrieren, dann Elemente verwalten.": {
        "en": "Register first, then manage elements.",
        "es": "Regístralo primero y luego gestiona los elementos.",
        "fr": "Enregistrez d'abord, puis gérez les éléments.",
    },
    "Erstes Byte (Offset)": {
        "en": "First byte (offset)",
        "es": "Primer byte (offset)",
        "fr": "Premier octet (offset)",
    },
    "Event: Name fehlt.": {
        "en": "Event: name is missing.",
        "es": "Evento: falta el nombre.",
        "fr": "Événement : nom manquant.",
    },
    "Events (beim Drücken)": {
        "en": "Events (on press)",
        "es": "Eventos (al pulsar)",
        "fr": "Événements (à l'appui)",
    },
    "Falls die Drehrichtungen vertauscht sind — tauscht die beiden Codes beim Speichern.": {
        "en": "If the turn directions are swapped — swaps the two codes on save.",
        "es": "Si los sentidos de giro están intercambiados — intercambia los dos códigos al guardar.",
        "fr": "Si les sens de rotation sont inversés — échange les deux codes à l'enregistrement.",
    },
    "Fertig": {"en": "Done", "es": "Listo", "fr": "Terminé"},
    "Fertig — Codes prüfen und übernehmen.": {
        "en": "Done — check the codes and apply.",
        "es": "Listo — comprueba los códigos y aplica.",
        "fr": "Terminé — vérifiez les codes et appliquez.",
    },
    "Flanken": {"en": "Edges", "es": "Flancos", "fr": "Fronts"},
    "Ganzes Panel (Knöpfe + Anzeigen):": {
        "en": "Whole panel (buttons + displays):",
        "es": "Panel completo (botones + indicadores):",
        "fr": "Panneau entier (boutons + afficheurs) :",
    },
    "Ganzes Panel in einem Rutsch anlegen — eine Vorlage aus Knöpfen + Anzeigen. Die Saitek-Panels sind eingebaut (hidraw); eigene Anordnungen als Vorlage speicherbar.": {
        "en": "Create a whole panel in one go — a template of buttons + displays. The Saitek panels are built in (hidraw); your own arrangements can be saved as a template.",
        "es": "Crea un panel completo de una vez — una plantilla de botones + indicadores. Los paneles Saitek vienen incorporados (hidraw); tus propias disposiciones se pueden guardar como plantilla.",
        "fr": "Créer un panneau entier d'un coup — un modèle de boutons + afficheurs. Les panneaux Saitek sont intégrés (hidraw) ; vos propres dispositions peuvent être enregistrées comme modèle.",
    },
    "Gerät deregistrieren": {
        "en": "Deregister device",
        "es": "Anular registro del dispositivo",
        "fr": "Désenregistrer l'appareil",
    },
    "Gerät getrennt.": {
        "en": "Device disconnected.",
        "es": "Dispositivo desconectado.",
        "fr": "Appareil déconnecté.",
    },
    "Gerät nicht lesbar.": {
        "en": "Device not readable.",
        "es": "Dispositivo no legible.",
        "fr": "Appareil illisible.",
    },
    ...
    "Geräte-Katalog nicht lesbar": {
        "en": "Device catalog not readable",
        "es": "Catálogo de dispositivos no legible",
        "fr": "Catalogue d'appareils illisible",
    },
    "Geräte-Paket": {
        "en": "Device package",
        "es": "Paquete de dispositivo",
        "fr": "Paquet d'appareil",
    },
    "Geräte-Paket exportieren": {
        "en": "Export device package",
        "es": "Exportar paquete de dispositivo",
        "fr": "Exporter le paquet d'appareil",
    },
    "Geräte-Paket importieren": {
        "en": "Import device package",
        "es": "Importar paquete de dispositivo",
        "fr": "Importer un paquet d'appareil",
    },
    "Geräte-Paket importieren…": {
        "en": "Import device package…",
        "es": "Importar paquete de dispositivo…",
    ...
    "Kein Profil geladen.": {
        "en": "No profile loaded.",
        "es": "Ningún perfil cargado.",
        "fr": "Aucun profil chargé.",
    },
    "Kein Zielprofil ausgewählt.": {
        "en": "No target profile selected.",
        "es": "Ningún perfil de destino seleccionado.",
        "fr": "Aucun profil cible sélectionné.",
    },
    "Kein anderes Profil hat Mappings für „{dev}“.": {
        "en": "No other profile has mappings for “{dev}”.",
        "es": "Ningún otro perfil tiene mapeos para «{dev}».",
        "fr": "Aucun autre profil n'a de mappages pour « {dev} ».",
    },
    "Kein anderes Profil vorhanden.": {
        "en": "No other profile available.",
        "es": "No hay otro perfil disponible.",
        "fr": "Aucun autre profil disponible.",
    },
    "Keine ausgeblendeten Geräte.": {
        "en": "No hidden devices.",
        "es": "No hay dispositivos ocultos.",
        "fr": "Aucun appareil masqué.",
    },
    "Klick auf ein Element öffnet den Editor · Rechtsklick: Bearbeiten / Duplizieren / Entfernen": {
        "en": "Click an element to open the editor · right-click: edit / duplicate / remove",
        "es": "Clic en un elemento abre el editor · clic derecho: editar / duplicar / quitar",
        "fr": "Cliquer sur un élément ouvre l'éditeur · clic droit : modifier / dupliquer / retirer",
    },
    "Klick: gemappt → Editor, leerer Platzhalter → neu mappen · Schalter/Achsen live": {
        "en": "Click: mapped → editor, empty placeholder → map anew · switches/axes live",
        "es": "Clic: mapeado → editor, hueco vacío → mapear de nuevo · interruptores/ejes en vivo",
        "fr": "Clic : mappé → éditeur, emplacement vide → remapper · interrupteurs/axes en direct",
    },
    "Konnte nicht laden: ": {
        "en": "Could not load: ",
    ...
    "Kurz-ID für dieses Gerät (Profile referenzieren sie):": {
        "en": "Short ID for this device (profiles reference it):",
        "es": "ID corto para este dispositivo (los perfiles lo referencian):",
        "fr": "ID court pour cet appareil (les profils le référencent) :",
    },
    "LED an, wenn Variable <Operator> Wert (z. B. ≥ 0.5, < 30, == 2, != 0). Wert leer lassen = diese Bedingung weglassen.": {
        "en": "LED on when variable <operator> value (e.g. ≥ 0.5, < 30, == 2, != 0). Leave the value empty = drop this condition.",
        "es": "LED encendido cuando variable <operador> valor (p. ej. ≥ 0.5, < 30, == 2, != 0). Deja el valor vacío = omitir esta condición.",
        "fr": "LED allumée quand variable <opérateur> valeur (p. ex. ≥ 0.5, < 30, == 2, != 0). Laisser la valeur vide = ignorer cette condition.",
    },
    "LED hinzufügen": {"en": "Add LED", "es": "Añadir LED", "fr": "Ajouter une LED"},
    "LED leuchtet, wenn ALLE gesetzten Bedingungen zutreffen. Eine reicht meist; zwei = Fenster, z. B. ≥ 0.01 UND < 0.95 für die rote Fahrwerks-LED beim Ausfahren.": {
        "en": "The LED is on when ALL set conditions hold. One is usually enough; two = a window, e.g. ≥ 0.01 AND < 0.95 for the red gear LED while extending.",
        "es": "El LED se enciende cuando se cumplen TODAS las condiciones definidas. Una suele bastar; dos = una ventana, p. ej. ≥ 0.01 Y < 0.95 para el LED rojo del tren al extenderse.",
        "fr": "La LED s'allume quand TOUTES les conditions définies sont vraies. Une suffit en général ; deux = une fenêtre, p. ex. ≥ 0.01 ET < 0.95 pour la LED rouge du train pendant la sortie.",
    },
    "LED…": {"en": "LED…", "es": "LED…", "fr": "LED…"},
    "Lampe": {"en": "Lamp", "es": "Lámpara", "fr": "Voyant"},
    "Lese- (Inputs) und Schreib-Elemente (Anzeigen) getrennt verwalten: Inputs am Gerät betätigen und benennen, Anzeigen (LEDs/Displays) hinzufügen.": {
        "en": "Manage read (inputs) and write elements (displays) separately: actuate and name inputs on the device, add displays (LEDs/displays).",
        "es": "Gestiona por separado los elementos de lectura (entradas) y de escritura (indicadores): acciona y nombra las entradas en el dispositivo, añade indicadores (LEDs/pantallas).",
        "fr": "Gérez séparément les éléments de lecture (entrées) et d'écriture (afficheurs) : actionnez et nommez les entrées sur l'appareil, ajoutez des afficheurs (LED/afficheurs).",
    },
    "Live-Anlernen braucht das angesteckte hidraw-Panel.": {
        "en": "Live teaching needs the connected hidraw panel.",
        "es": "El aprendizaje en vivo necesita el panel hidraw conectado.",
        "fr": "L'apprentissage en direct nécessite le panneau hidraw connecté.",
    },
    "Live-Anlernen geht nur für angesteckte evdev-Geräte (Yoke/Pedale/Quadrant).": {
        "en": "Live teaching only works for a connected evdev device (yoke/pedals/quadrant).",
        "es": "El aprendizaje en vivo solo funciona con un dispositivo evdev conectado (yugo/pedales/cuadrante).",
        "fr": "L'apprentissage en direct ne fonctionne qu'avec un appareil evdev connecté (yoke/palonnier/quadrant).",
    },
    "Live-Vorschau (Nadel bei ~65 %)": {
        "en": "Live preview (needle at ~65%)",
        "es": "Vista previa en vivo (aguja al ~65 %)",
        "fr": "Aperçu en direct (aiguille à ~65 %)",
    },
    "MSFS-Prefix automatisch finden (wie tools/find-prefix.sh)": {
        "en": "Find the MSFS prefix automatically (like tools/find-prefix.sh)",
        "es": "Encuentra el prefix de MSFS automáticamente (como tools/find-prefix.sh)",
        "fr": "Trouver le préfixe MSFS automatiquement (comme tools/find-prefix.sh)",
    },
    "Mapping entfernen": {"en": "Remove mapping", "es": "Quitar mapeo", "fr": "Retirer le mappage"},
    "Mappings aus einem anderen Profil holen…": {
        "en": "Pull mappings from another profile…",
        "es": "Traer mapeos de otro perfil…",
        "fr": "Récupérer les mappages d'un autre profil…",
    },
    "Mappings holen": {"en": "Pull mappings", "es": "Traer mapeos", "fr": "Récupérer les mappages"},
    "Mappings in anderes Profil übertragen…": {
        "en": "Transfer mappings to another profile…",
        "es": "Transferir mapeos a otro perfil…",
        "fr": "Transférer les mappages vers un autre profil…",
    },
    "Mappings übertragen": {
        "en": "Transfer mappings",
        "es": "Transferir mapeos",
        "fr": "Transférer les mappages",
    },
    "Mehrere Events": {
        "en": "Multiple events",
        "es": "Varios eventos",
        "fr": "Plusieurs événements",
    },
    "Mehrere Events je Betätigung (Event feuern / Variable setzen):": {
        "en": "Multiple events per actuation (fire event / set variable):",
        "es": "Varios eventos por accionamiento (disparar evento / fijar variable):",
        "fr": "Plusieurs événements par action (déclencher un événement / définir une variable) :",
    },
    "Mindestens ein Event beim Drücken/Einschalten nötig.": {
        "en": "At least one event on press/switch-on is required.",
        "es": "Se requiere al menos un evento al pulsar/encender.",
        "fr": "Au moins un événement à l'appui/à l'allumage est requis.",
    },
    "Mindestens eine Bedingung mit einem Wert angeben.": {
        "en": "Specify at least one condition with a value.",
        "es": "Especifica al menos una condición con un valor.",
        "fr": "Indiquez au moins une condition avec une valeur.",
    },
    ...
    "Nichts hinzuzufügen — alle Vorlage-Elemente sind schon da.": {
        "en": "Nothing to add — all template elements are already there.",
        "es": "Nada que añadir — todos los elementos de la plantilla ya están.",
        "fr": "Rien à ajouter — tous les éléments du modèle sont déjà là.",
    },
    "Panel testen": {"en": "Test panel", "es": "Probar panel", "fr": "Tester le panneau"},
    "Positionen": {"en": "Positions", "es": "Posiciones", "fr": "Positions"},
    "Probiert die LEDs/Zellen am echten Panel durch, bis deine aufleuchtet — dann werden Byte/Bit bzw. Zelle automatisch eingetragen.": {
        "en": "Steps through the LEDs/cells on the real panel until yours lights up — then byte/bit or cell is filled in automatically.",
        "es": "Recorre los LEDs/celdas del panel real hasta que se encienda el tuyo — entonces se rellena byte/bit o celda automáticamente.",
        "fr": "Parcourt les LED/cellules du vrai panneau jusqu'à ce que la vôtre s'allume — puis octet/bit ou cellule est renseigné automatiquement.",
    },
    "Registriert ✓ → ": {"en": "Registered ✓ → ", "es": "Registrado ✓ → ", "fr": "Enregistré ✓ → "},
    "Report-Byte": {"en": "Report byte", "es": "Byte del reporte", "fr": "Octet du rapport"},
    "Report-Bytes:": {
        "en": "Report bytes:",
        "es": "Bytes del reporte:",
        "fr": "Octets du rapport :",
    },
    "Richtung": {"en": "Direction", "es": "Dirección", "fr": "Direction"},
    "SPAD.neXt-Import": {
        "en": "SPAD.neXt import",
        "es": "Importación de SPAD.neXt",
        "fr": "Importation SPAD.neXt",
    },
    "SPAD.neXt-Profil (.xml) oder Katalog (.json) wählen": {
        "en": "Choose a SPAD.neXt profile (.xml) or catalog (.json)",
        "es": "Elige un perfil SPAD.neXt (.xml) o un catálogo (.json)",
        "fr": "Choisir un profil SPAD.neXt (.xml) ou un catalogue (.json)",
    },
    "SWAP-Taster": {"en": "SWAP button", "es": "Botón SWAP", "fr": "Bouton SWAP"},
    "Schalter mehrmals umlegen.": {
        "en": "Flip the switch several times.",
        "es": "Cambia el interruptor varias veces.",
        "fr": "Basculez l'interrupteur plusieurs fois.",
    },
    ...
    "Totzone (roh)": {
        "en": "Deadzone (raw)",
        "es": "Zona muerta (bruto)",
        "fr": "Zone morte (brut)",
    },
    "Totzone muss innerhalb Eingang min…max liegen ({lo}…{hi}).": {
        "en": "Deadzone must be within input min…max ({lo}…{hi}).",
        "es": "La zona muerta debe estar dentro del min…max de entrada ({lo}…{hi}).",
        "fr": "La zone morte doit être dans le min…max d'entrée ({lo}…{hi}).",
    },
    "Totzone: max muss größer als min sein.": {
        "en": "Deadzone: max must be greater than min.",
        "es": "Zona muerta: max debe ser mayor que min.",
        "fr": "Zone morte : max doit être supérieur à min.",
    },
    "Totzone: min UND max angeben (oder beide leer).": {
        "en": "Deadzone: specify min AND max (or leave both empty).",
        "es": "Zona muerta: indica min Y max (o deja ambos vacíos).",
        "fr": "Zone morte : indiquez min ET max (ou laissez les deux vides).",
    },
    "Trimmrad": {"en": "Trim wheel", "es": "Rueda de compensación", "fr": "Molette de trim"},
    "Variable fehlt.": {
        "en": "Variable is missing.",
        "es": "Falta la variable.",
        "fr": "Variable manquante.",
    },
    "Vorhandene Elemente bleiben erhalten.": {
        "en": "Existing elements are kept.",
        "es": "Los elementos existentes se conservan.",
        "fr": "Les éléments existants sont conservés.",
    },
    "Vorlage ▾": {"en": "Template ▾", "es": "Plantilla ▾", "fr": "Modèle ▾"},
    "Weiter": {"en": "Next", "es": "Siguiente", "fr": "Suivant"},
    "Welche einzelne LED innerhalb des Bytes. Jedes Byte steuert bis zu 8 Lampen (Bit 0-7). Auch das findet „🔦 Byte/Bit suchen…“ automatisch.": {
        "en": "Which single LED within the byte. Each byte drives up to 8 lamps (bit 0-7). “🔦 Find byte/bit…” finds this automatically too.",
        "es": "Qué LED individual dentro del byte. Cada byte controla hasta 8 lámparas (bit 0-7). «🔦 Buscar byte/bit…» también lo encuentra automáticamente.",
        "fr": "Quelle LED individuelle dans l'octet. Chaque octet pilote jusqu'à 8 voyants (bit 0-7). « 🔦 Trouver octet/bit… » le trouve aussi automatiquement.",
    },
    "Wert-Encoder": {"en": "Value encoder", "es": "Encoder de valor", "fr": "Encodeur de valeur"},
    "Wie heißt diese Anzeige?": {
        "en": "What's this display called?",
        "es": "¿Cómo se llama este indicador?",
        "fr": "Comment s'appelle cet afficheur ?",
    },
    "Wie heißt dieser Encoder? (z. B. „Heading“, „Höhe“)": {
        "en": "What's this encoder called? (e.g. “Heading”, “Altitude”)",
        "es": "¿Cómo se llama este encoder? (p. ej. «Rumbo», «Altitud»)",
        "fr": "Comment s'appelle cet encodeur ? (p. ex. « Cap », « Altitude »)",
    },
    "Wie heißt dieses Bedienelement?": {
        "en": "What's this control called?",
        "es": "¿Cómo se llama este control?",
        "fr": "Comment s'appelle cette commande ?",
    },
    "Wie viele Stellen/Segmente hat das Display?": {
        "en": "How many digits/segments does the display have?",
        "es": "¿Cuántos dígitos/segmentos tiene la pantalla?",
        "fr": "Combien de chiffres/segments a l'afficheur ?",
    },
    "Wie viele Ziffern die Anzeige hat (z. B. 5 für eine Frequenz wie 118.00).": {
        "en": "How many digits the display has (e.g. 5 for a frequency like 118.00).",
        "es": "Cuántos dígitos tiene el indicador (p. ej. 5 para una frecuencia como 118.00).",
        "fr": "Combien de chiffres a l'afficheur (p. ex. 5 pour une fréquence comme 118.00).",
    },
    "Wieder anzeigen": {"en": "Show again", "es": "Mostrar de nuevo", "fr": "Réafficher"},
    "Zahlenfeld ungültig.": {
        "en": "Number field invalid.",
        "es": "Campo numérico no válido.",
        "fr": "Champ numérique non valide.",
    },
    "Zeiger": {"en": "Needle", "es": "Aguja", "fr": "Aiguille"},
    "Zellen": {"en": "Cells", "es": "Celdas", "fr": "Cellules"},
    "Ziffern nach dem Komma (0 = ganze Zahl).": {
        "en": "Digits after the decimal point (0 = whole number).",
        "es": "Dígitos tras la coma (0 = número entero).",
        "fr": "Chiffres après la virgule (0 = nombre entier).",
    },
    "Zweite Bedingung, UND-verknüpft — für ein Fenster. Leer = nicht benutzt.": {
        "en": "Second condition, AND-combined — for a window. Empty = unused.",
        "es": "Segunda condición, combinada con Y — para una ventana. Vacío = sin usar.",
        "fr": "Deuxième condition, combinée par ET — pour une fenêtre. Vide = inutilisé.",
    },
    "alles aus": {"en": "stop all", "es": "detener todo", "fr": "tout arrêter"},
    "anlernen": {"en": "teach", "es": "aprender", "fr": "apprendre"},
    "außen": {"en": "outer", "es": "exterior", "fr": "extérieur"},
    "bash bridge/run-bridge.sh   (Supervisor → bridge.py → Proton)": {
        "en": "bash bridge/run-bridge.sh   (supervisor → bridge.py → Proton)",
        "es": "bash bridge/run-bridge.sh   (supervisor → bridge.py → Proton)",
        "fr": "bash bridge/run-bridge.sh   (superviseur → bridge.py → Proton)",
    },
    "bash bridge/setup-prefix.sh  (Windows-Python + SimConnect)": {
        "en": "bash bridge/setup-prefix.sh  (Windows Python + SimConnect)",
        "es": "bash bridge/setup-prefix.sh  (Python de Windows + SimConnect)",
        "fr": "bash bridge/setup-prefix.sh  (Python Windows + SimConnect)",
    },
    "das Trimmrad mehrmals nach oben / im Uhrzeigersinn drehen": {
        "en": "turn the trim wheel up / clockwise several times",
        "es": "gira la rueda de compensación hacia arriba / en sentido horario varias veces",
        "fr": "tournez la molette de trim vers le haut / dans le sens horaire plusieurs fois",
    },
    "das Trimmrad mehrmals nach unten / gegen den Uhrzeigersinn drehen": {
        "en": "turn the trim wheel down / counter-clockwise several times",
        "es": "gira la rueda de compensación hacia abajo / en sentido antihorario varias veces",
        "fr": "tournez la molette de trim vers le bas / dans le sens antihoraire plusieurs fois",
    },
    "den INNEREN Ring mehrmals GEGEN den Uhrzeigersinn drehen": {
        "en": "turn the INNER ring COUNTER-clockwise several times",
        "es": "gira el anillo INTERIOR en sentido ANTIHORARIO varias veces",
        "fr": "tournez l'anneau INTÉRIEUR dans le sens ANTIHORAIRE plusieurs fois",
    },
    "den INNEREN Ring mehrmals IM Uhrzeigersinn drehen": {
        "en": "turn the INNER ring CLOCKWISE several times",
        "es": "gira el anillo INTERIOR en sentido HORARIO varias veces",
        "fr": "tournez l'anneau INTÉRIEUR dans le sens HORAIRE plusieurs fois",
    },
    "den Mode-Selektor mehrmals auf DIESE Position drehen": {
        "en": "turn the mode selector to THIS position several times",
        "es": "gira el selector de modo a ESTA posición varias veces",
        "fr": "tournez le sélecteur de mode sur CETTE position plusieurs fois",
    },
    "den SWAP-Taster mehrmals drücken": {
        "en": "press the SWAP button several times",
        "es": "pulsa el botón SWAP varias veces",
        "fr": "appuyez plusieurs fois sur le bouton SWAP",
    },
    "den ÄUSSEREN Ring mehrmals GEGEN den Uhrzeigersinn drehen": {
        "en": "turn the OUTER ring COUNTER-clockwise several times",
        "es": "gira el anillo EXTERIOR en sentido ANTIHORARIO varias veces",
        "fr": "tournez l'anneau EXTÉRIEUR dans le sens ANTIHORAIRE plusieurs fois",
    },
    "den ÄUSSEREN Ring mehrmals IM Uhrzeigersinn drehen": {
        "en": "turn the OUTER ring CLOCKWISE several times",
        "es": "gira el anillo EXTERIOR en sentido HORARIO varias veces",
        "fr": "tournez l'anneau EXTÉRIEUR dans le sens HORAIRE plusieurs fois",
    },
    "erkannt": {"en": "detected", "es": "detectado", "fr": "détecté"},
    "filedialog.askdirectory → prefix_path (gui-settings.json)": {
        "en": "filedialog.askdirectory → prefix_path (gui-settings.json)",
        "es": "filedialog.askdirectory → prefix_path (gui-settings.json)",
        "fr": "filedialog.askdirectory → prefix_path (gui-settings.json)",
    },
    ...
    "killpg SIGTERM (Mapper-Prozessgruppe) + sweep 'peripherals_bridge run'": {
        "en": "killpg SIGTERM (mapper process group) + sweep 'peripherals_bridge run'",
        "es": "killpg SIGTERM (grupo de procesos del mapeador) + barrido 'peripherals_bridge run'",
        "fr": "killpg SIGTERM (groupe de processus du mappeur) + balayage 'peripherals_bridge run'",
    },
    "killpg SIGTERM + sweep 'bridge/bridge.py' / 'bridge/run-bridge.sh'": {
        "en": "killpg SIGTERM + sweep 'bridge/bridge.py' / 'bridge/run-bridge.sh'",
        "es": "killpg SIGTERM + barrido 'bridge/bridge.py' / 'bridge/run-bridge.sh'",
        "fr": "killpg SIGTERM + balayage 'bridge/bridge.py' / 'bridge/run-bridge.sh'",
    },
    "leer · Klick zum Mappen": {
        "en": "empty · click to map",
        "es": "vacío · clic para mapear",
        "fr": "vide · cliquer pour mapper",
    },
    "leuchtet ~2 s": {"en": "lights ~2 s", "es": "se enciende ~2 s", "fr": "s'allume ~2 s"},
    "mehrere Events je Betätigung — unten bearbeiten ↓": {
        "en": "multiple events per actuation — edit below ↓",
        "es": "varios eventos por accionamiento — editar abajo ↓",
        "fr": "plusieurs événements par action — modifier ci-dessous ↓",
    },
    "nein": {"en": "no", "es": "no", "fr": "non"},
    "nicht gefunden — angesteckt?": {
        "en": "not found — plugged in?",
        "es": "no encontrado — ¿conectado?",
        "fr": "introuvable — branché ?",
    },
    ...
    "pkexec tools/install-udev-rules.sh  (udev-Regeln, Passwortdialog)": {
        "en": "pkexec tools/install-udev-rules.sh  (udev rules, password dialog)",
        "es": "pkexec tools/install-udev-rules.sh  (reglas udev, diálogo de contraseña)",
        "fr": "pkexec tools/install-udev-rules.sh  (règles udev, dialogue de mot de passe)",
    },
    "prefix_path speichern + Bridge-Env (STEAM_COMPAT_DATA_PATH) setzen": {
        "en": "save prefix_path + set bridge env (STEAM_COMPAT_DATA_PATH)",
        "es": "guardar prefix_path + fijar el entorno del bridge (STEAM_COMPAT_DATA_PATH)",
        "fr": "enregistrer prefix_path + définir l'env du bridge (STEAM_COMPAT_DATA_PATH)",
    },
    "registriert als ": {
        "en": "registered as ",
        "es": "registrado como ",
    ...
    "stop_mapper() + stop_bridge() — alle Strays wegräumen": {
        "en": "stop_mapper() + stop_bridge() — clear all strays",
        "es": "stop_mapper() + stop_bridge() — eliminar todos los restos",
        "fr": "stop_mapper() + stop_bridge() — nettoyer tous les résidus",
    },
    "weiterdrehen oder «Weiter»": {
        "en": "keep turning or «Next»",
        "es": "sigue girando o «Siguiente»",
        "fr": "continuez à tourner ou « Suivant »",
    },
    "{n} Variablen/Events aus {src} geladen (davon {ev} Events). Sie stehen jetzt ganz oben im Variablen-Picker beim Mappen.": {
        "en": "Loaded {n} variables/events from {src} (of which {ev} events). They now sit at the top of the variable picker when mapping.",
        "es": "Se cargaron {n} variables/eventos de {src} (de ellos {ev} eventos). Ahora aparecen arriba en el selector de variables al mapear.",
        "fr": "{n} variables/événements chargés depuis {src} (dont {ev} événements). Ils apparaissent maintenant en haut du sélecteur de variables lors du mappage.",
    },
    ...
    "— warte auf Bewegung —": {
        "en": "— waiting for movement —",
        "es": "— esperando movimiento —",
        "fr": "— en attente de mouvement —",
    },
    "„{dev}“ ({nb} Eingaben / {no} Anzeigen) übertragen nach:": {
        "en": "Transfer “{dev}” ({nb} inputs / {no} displays) to:",
        "es": "Transferir «{dev}» ({nb} entradas / {no} indicadores) a:",
        "fr": "Transférer « {dev} » ({nb} entrées / {no} afficheurs) vers :",
    },
    ...
    "„{dev}“ importiert ✓\n\n{nb} Eingaben / {no} Anzeigen{into} · Anordnung: {lay} · Kalibrierung: {cal}": {
        "en": "“{dev}” imported ✓\n\n{nb} inputs / {no} displays{into} · arrangement: {lay} · calibration: {cal}",
        "es": "«{dev}» importado ✓\n\n{nb} entradas / {no} indicadores{into} · disposición: {lay} · calibración: {cal}",
        "fr": "« {dev} » importé ✓\n\n{nb} entrées / {no} afficheurs{into} · disposition : {lay} · étalonnage : {cal}",
    },
    "„{dev}“ — Mappings holen aus:": {
        "en": "“{dev}” — pull mappings from:",
        "es": "«{dev}» — traer mapeos de:",
        "fr": "« {dev} » — récupérer les mappages de :",
    },
    "„{dev}“ → „{t}“: {wb} Eingaben / {wo} Anzeigen übertragen ✓": {
        "en": "“{dev}” → “{t}”: {wb} inputs / {wo} displays transferred ✓",
        "es": "«{dev}» → «{t}»: {wb} entradas / {wo} indicadores transferidos ✓",
        "fr": "« {dev} » → « {t} » : {wb} entrées / {wo} afficheurs transférés ✓",
    },
    "„{dev}“: {wb} Eingaben / {wo} Anzeigen aus „{s}“ geholt ✓": {
        "en": "“{dev}”: {wb} inputs / {wo} displays pulled from “{s}” ✓",
        "es": "«{dev}»: {wb} entradas / {wo} indicadores traídos de «{s}» ✓",
        "fr": "« {dev} » : {wb} entrées / {wo} afficheurs récupérés de « {s} » ✓",
    },
    "„{name}“ aus der Geräteliste entfernen?\n\nDas blendet das Gerät nur für dich aus (der mitgelieferte Katalog und deine Profil-Mappings bleiben unangetastet). Über den Geräte-Explorer → „Ausgeblendete Geräte…“ kannst du es jederzeit wieder einblenden.": {
        "en": "Remove “{name}” from the device list?\n\nThis only hides the device for you (the bundled catalog and your profile mappings stay untouched). Via Device Explorer → “Hidden devices…” you can show it again any time.",
        "es": "¿Quitar «{name}» de la lista de dispositivos?\n\nEsto solo oculta el dispositivo para ti (el catálogo incluido y tus mapeos de perfil quedan intactos). Desde el Explorador de dispositivos → «Dispositivos ocultos…» puedes volver a mostrarlo cuando quieras.",
        "fr": "Retirer « {name} » de la liste des appareils ?\n\nCela masque l'appareil uniquement pour vous (le catalogue fourni et vos mappages de profil restent intacts). Via l'Explorateur d'appareils → « Appareils masqués… », vous pouvez le réafficher à tout moment.",
    },
    "„{name}“ deregistriert ✓": {
        "en": "“{name}” deregistered ✓",
        "es": "«{name}» dado de baja ✓",
        "fr": "« {name} » désenregistré ✓",
    },
    "„{t}“ hat für dieses Gerät schon {eb} Eingaben / {eo} Anzeigen. Überschreiben?": {
        "en": "“{t}” already has {eb} inputs / {eo} displays for this device. Overwrite?",
        "es": "«{t}» ya tiene {eb} entradas / {eo} indicadores para este dispositivo. ¿Sobrescribir?",
        "fr": "« {t} » a déjà {eb} entrées / {eo} afficheurs pour cet appareil. Écraser ?",
    },
    "• (kein Mapping im Paket)": {
        "en": "• (no mapping in the package)",
        "es": "• (sin mapeo en el paquete)",
        "fr": "• (aucun mappage dans le paquet)",
    },
    "• Mapping landet in Profil „{p}“ (überschreibt vorhandenes)": {
        "en": "• Mapping goes into profile “{p}” (overwrites existing)",
        "es": "• El mapeo va al perfil «{p}» (sobrescribe el existente)",
        "fr": "• Le mappage va dans le profil « {p} » (écrase l'existant)",
    },
    "● live": {"en": "● live", "es": "● en vivo", "fr": "● en direct"},
    "⚠ Der Mapper läuft und steuert das Panel — bitte erst im Connection-Tab stoppen.": {
        "en": "⚠ The mapper is running and driving the panel — please stop it first in the Connection tab.",
        "es": "⚠ El mapeador está en marcha y controla el panel — deténlo primero en la pestaña Conexión.",
        "fr": "⚠ Le mappeur tourne et pilote le panneau — arrêtez-le d'abord dans l'onglet Connexion.",
    },
    "⚠ Der Mapper läuft — erst im Connection-Tab stoppen.": {
        "en": "⚠ The mapper is running — stop it first in the Connection tab.",
        "es": "⚠ El mapeador está en marcha — deténlo primero en la pestaña Conexión.",
        "fr": "⚠ Le mappeur tourne — arrêtez-le d'abord dans l'onglet Connexion.",
    },
    ...
    "🔦 Byte/Bit suchen…": {
        "en": "🔦 Find byte/bit…",
        "es": "🔦 Buscar byte/bit…",
        "fr": "🔦 Trouver octet/bit…",
    },
    "🔦 LEDs/Display testen…": {
        "en": "🔦 Test LEDs/display…",
        "es": "🔦 Probar LEDs/pantalla…",
        "fr": "🔦 Tester LED/afficheur…",
    },
    "🔦 Zelle suchen…": {
        "en": "🔦 Find cell…",
        "es": "🔦 Buscar celda…",
