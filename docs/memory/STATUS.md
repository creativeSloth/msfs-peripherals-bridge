...
> **Dann Doku: User wollte EIN zusammenhängendes Handbuch mit ALLEM (Install-Skript, Prefix, evdev/hidraw, Profil-Transfer, GUI+Terminal-Wege, für Nicht-Programmierer) + „räum die Doku auf, prüf README-Links, halt es schlank".** Gebaut **`docs/HANDBUCH.md`** (DEUTSCH, das eine End-to-End-Manual: Schnellstart 4 Befehle → Terminal/Install → udev (GUI-Button + Terminal) → **evdev vs hidraw erklärt** → fremde HW registrieren/anlernen/deregistrieren → Mappen ohne Sim → **Profile: Transfer/Geräte-Paket/Backup** → Bridge/Prefix (Steam-Varianten) → Fliegen → Alltag/Troubleshooting/Datenorte; jeder Schritt 🖱 App + ⌨ Terminal, ✓-Checkpoints). **Konsolidiert:** `QUICKSTART.md` + `INSTALL.md` + `running.md` **GELÖSCHT** (voll ins Handbuch aufgegangen), alle eingehenden Links umgebogen (README top + docs/README-Index neu, `install.sh`, `cli.py`-Docstring, `cheatsheet.md`, `geraete-baukasten-konzept.md`). `geraete-workflow.md` bleibt als Design/Status-Referenz (Concept-Sektion). Link-Checker: alle relativen MD-Links in README/docs/HANDBUCH grün.
> **Nächstes:** committen+pushen (User: „mach das bitte"); GUI der beiden Features + Handbuch-Genauigkeit am Display gegenlesen.
>
