...
>
> ## CUT 2026-08-07 — SCHRITT A + evdev-SCAN KOMPLETT (Achsen + Buttons/Switch/Encoder), 394 Tests grün, ruff clean
> **COMMITTET (nicht gepusht):** `0c84e58` Schritt A (`template_elements`) · `d9c215b` evdev-Achsen-Capture. Danach evdev-Button/Switch/Encoder-Capture gebaut (③, uncommitted bis zum nächsten Commit).
> **③ evdev-Button/Switch/Encoder-Capture (NEU, uncommitted):** reines **`evdev_reader.key_edges(last, events)`** (zählt 0→1-Press-Flanken je Keycode, ignoriert Halten/Autorepeat/Release — Pendant zu `hidraw_reader.count_rising_edges`) + evdev **`button_edge_reader(path)`** (`(read,close)`, draint EV_KEY nicht-blockierend, kumuliert Flanken; Gewinner via `hidraw_reader.winning_code`). **GUI:** `_capture` ist jetzt **reader-agnostisch** (`opener`-Param, Default = hidraw `edge_count_reader`); `_add_input` routet bei `transport==evdev` Button/Schalter/Encoder über `button_edge_reader`. Damit ist der Wizard für FRISCHE evdev-Geräte komplett (Achsen ②, Buttons/Switch/Encoder ③). 4 Tests. **⚠️ Rest-Lücke:** ein evdev-Encoder, der als REL/Achse statt als Key-Events meldet, wird von `button_edge_reader` nicht erkannt (Saitek/Yoke melden Buttons als Keys → passt; exotische Encoder = Follow-up).
...
> **🔴 SOFORT nächste Session:** (1) **③ committen + alle pushen** (User fragen). (2) **VISUELL prüfen** (Display+HW): Mapper → „🔍 Geräte-Explorer…" → Gerät → „Geräteelemente…" → **„Aus Vorlage füllen…"** (landen Encoder/Selektor/LEDs/Display in den 2 Baum-Gruppen?) und **„+ Input anlernen…"** an Yoke/Pedale — **Achse** (bewegte Achse + Range erkannt?) **und Taster/Schalter** (richtiger Code erkannt?). (3) **Schritt E** (Nordstern-Rest) — User wählte aber zunächst „kleinere Follow-ups", also ggf. erst: benannte Inputs im Nachbau (Schritt C-b), Doppelzählung sauber, Output-Scan (D).
