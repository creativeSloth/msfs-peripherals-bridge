...
> - ✅ **ATOMARE STATISTIK GEBAUT** (User: „von Templates/Tabelle weg; Übersicht soll ATOMARE Input/Output-Elemente zählen, aktuell sucht sie Oberelemente", 379 Tests grün): reine **`gui_mapper.atomic_output_count(outputs)`** (Summe `panel_probe.probe_targets` = einzelne LEDs+Display-Zellen, nicht Blöcke) + **`atomic_input_count(ddef,profile)`** (Bindings + Controller-Input-Codes + gescannte `ddef.inputs`, Encoder = 1 Element). `build_device_rows` nutzt beide → „Out"-Spalte zeigt jetzt z. B. Multi=18 (8 LED+10 Zellen) statt „1 Block". 2 neue Tests + 1 angepasst. **▶ GROSSE VISION dok. (docs/geraete-baukasten-konzept.md §„Nachbau als Editor"): Nachbau wird primäre/einzige Oberfläche (Tabelle weg), generisches Hinzufügen von Anzeigen (LED/Display mit Art+Segmentzahl, zellenweise wie DME) + Inputs, toggle-barer Bearbeitungsmodus (verschieben/größen/Sinngruppen-Labels), ein-/ausklappbares Bearbeitungsmenü rechts — großer Batch NACH Output-Modellen (E).**
...
> **🔴 TODO nächste Session:** (1) **Geräte-Explorer + Eingänge-scannen visuell am Display prüfen** (Mapper → „🔍 Geräte-Explorer…" → Gerät registrieren → „Eingänge scannen…" → Knopf/Encoder am Saitek anlernen → landen die Aliase in der Liste/Overlay?). (2) **Schritt C:** die gescannten InputBlocks NUTZBAR machen — (a) im Nachbau/Mapper anzeigen, (b) beim Binding-Anlegen als benannte Quelle vorbelegen (heute referenzieren Bindings rohe Codes; Alias→Code-Brücke), (c) Achsen-Kalibrierung (evdev-Achsen-Capture, aktuell nur hidraw). (3) **Schritt D:** Output-Scan (LEDs/Display durchprobieren, generalisiertes `panel_probe`). (4) **Schritt E:** generische Output-Modelle (LED-aus-Sim + Display-Werte datengetrieben) = Paritäts-Knackpunkt. (5) Monitor-Sim-Prüf-Konzept. (6) Nachbau visuell/HW sichten. Optional zurückgestellt: Arduino/seriell.
> ---
