...
> **COMMITTET & GEPUSHT** (`feat/mapper-panel-nachbau`, ab `9a52c35` origin): `0c84e58` Schritt A (`template_elements`) · `d9c215b` evdev-Achsen-Capture · `095c182` evdev-Button/Switch/Encoder-Capture · `f19f6c9` STATUS · `de6a64d` Sicherung+Tools. **UNCOMMITTED bis nächster Commit:** Mapper-Knopf-Umbau (④).
...
> **🔴 SOFORT nächste Session:** (1) **VISUELL prüfen** (Display+HW): Mapper → „🔍 Geräte-Explorer…" → Gerät → „Geräteelemente…" → **„Aus Vorlage füllen…"** (landen Encoder/Selektor/LEDs/Display in den 2 Baum-Gruppen?) und **„+ Input anlernen…"** an Yoke/Pedale — **Achse** (bewegte Achse + Range erkannt?) **und Taster/Schalter** (richtiger Code erkannt?). (2) User wählte „**kleinere Follow-ups**" (nicht gleich Schritt E): offen = benannte Inputs im Nachbau (**Schritt C-b**), Doppelzählung sauber, Output-Scan (**D**). (3) **Schritt E** (Nordstern-Rest) danach.
