...
>
> ## CUT 2026-08-28 — INSTALL-FIX: **Tk** (die eine GUI-Abhängigkeit, die `uv` NICHT liefern kann) + TERMINAL-TEXTE ENGLISCH + `ruff format` (CI war rot), 479 Tests grün, committet+gepusht auf `main`
> **Fremd-User-Report (Arch): `./install.sh` lief sauber durch, dann `uv run python -m msfs_peripherals_bridge.gui` → `libtk8.6.so: cannot open shared object file`.** Ursache: **Tk ist nicht Teil von Python** — und der Installer hat es nie installiert. Zwei Ausprägungen, ein Paket: System-Python fehlt das Modul `tkinter`; der von `uv` mitgebrachte python-build-standalone HAT `_tkinter`, lädt aber `libtk8.6.so`/`libtcl8.6.so` beim Import aus der Distribution.
> - **`install.sh`**: Paketmanager wird **einmal** erkannt (`PKGMGR` = apt/dnf/pacman, `install_pkgs` als `case`), daraus die Tk-Pakete: apt **`python3-tk`** (zieht `libtk8.6`+`libtcl8.6` inkl. **unversionierter** `.so` — genau das, was der uv-Python braucht), dnf **`python3-tkinter tk`**, pacman **`tk`** (zieht tcl). Sie hängen jetzt in den Basis-Tools (Schritt 1). **Neuer Schritt 4 nach `uv sync`**: `.venv/bin/python -c 'import tkinter'` = der einzige ehrliche Test (prüft GENAU den Python, der die GUI startet; braucht **kein** Display, weil nur importiert und kein `Tk()` gebaut wird). Schlägt er fehl → Nachinstallation → sonst Ausgabe der exakten Befehle pro Distro + „die CLI funktioniert auch ohne Tk". udev ist dadurch Schritt 5. Alle drei Zweige (Tk fehlt / Tk da / unbekannte Distro mit leerem `tk_pkgs`) unter `set -euo pipefail` durchsimuliert — das Skript bricht nirgends ab.
> - **`gui.py`**: neu `tk_install_hint()` (`shutil.which` apt-get/dnf/pacman/zypper → passender Befehl); `main()` trennt **`ImportError`** (Tk fehlt — Modul ODER Lib, gleiche Kur) von allem anderen (Display). Vorher riet die Meldung **pauschal `python3-tk`** — auf Arch schlicht falsch.
> - **Terminal-Texte ENGLISCH** (User: „andere sollen das repo und das skript verstehen"): die Startmeldung bewusst **ohne i18n** (sie fällt an, bevor irgendein Fenster oder eine Sprachwahl existiert — DE-Default hätte den Fremd-User wieder ausgesperrt); dazu die letzten **11 deutschen `--help`/Ausgabe-Strings in `cli.py`** (export/import-device, export/import-config) und `tools/simulate-from-scratch.sh`. `install.sh`, `bridge/*.sh` und die übrigen Tools waren schon englisch.
> - **Doku**: README (Abschnitt „by hand") + MANUAL **§1** und **§12 Troubleshooting** nennen die Tk-Anforderung samt Befehl pro Distro.
> - **Tests +7** (`tests/test_gui_startup.py`, 472→**479**): Hint pro Paketmanager inkl. Fallback ohne bekannten Manager · `ImportError`-Pfad druckt Ursache **und** Befehl **und** `./install.sh` · Nicht-Import-Fehler zeigt DISPLAY statt Paket. mypy sauber.
> - **Zweiter Commit = reines `ruff format`**: `ruff format --check` (läuft in CI) war **seit dem i18n-Sweep ROT** — `gui.py`, `i18n.py`, `panel_layout.py` waren nie durch den Formatter gelaufen. Bewusst als eigener Commit, damit der Fix-Diff lesbar bleibt.
> - **Commits:** `1a2fb60` fix(install) · `da30afb` style — beide auf `main` gepusht.
> **🔴 Offen:** GUI weiterhin visuell UNGEPRÜFT (unverändert); der Tk-Pfad selbst ist nur **simuliert** — die hiesige Mint-Box hat Tk und ein `.venv` auf dem System-Python, ein echter Arch-Durchlauf steht aus (der meldende User ist die Gegenprobe).
>
> ## CUT 2026-08-23 (④) — ANORDNUNGSMODUS-EDITOR (Dekos + Entfernen + Banner-i18n) + GRID-FIX + DEVICE-EXPLORER-HINWEIS, 472 Tests grün, ruff sauber (committet+gepusht)
> **Erst der LED-Operatoren-Batch (③ unten) committet+gepusht (`5674347`), dabei 2 RUF001/E501-Altlasten aus dem Commit mitgefixt.** Dann mehrteiliger Anordnungsmodus-Auftrag (Mapper-Nachbau `gui.py` + `mapping/loader.py`):
> - **Banner-Überschriften (Schalter/Magnetos/Fahrwerk/Weitere Anzeigen) jetzt i18n** — Label bleibt kanonisch (stabiler Override-Key), Anzeige via `tr()` im Render (`_draw_header`/`_panel_hover`).
> - **Dekorationen „+ Deko"** (Box hinter Knöpfe / Trennlinie / Textlabel): eigener Store `decorations:` in `panel-layouts.yaml` (`load/save_panel_decorations`), Render (Box `tag_lower` hinter Controls, Linie/Label drüber), Drag/Resize/Text-bearbeiten/Löschen im Kontextmenü.
> - **Elemente einzeln per Rechtsklick „Entfernen"** (non-destruktiv = Hide-Schicht `hidden:` pro Gerät, Mapping bleibt; „Ausgeblendete einblenden" + ↺ holen zurück). **Banner umbenennen** (`labels:`-Override, display-only). `clear_panel_layout` wischt alle 4 Sektionen.
> - **„Raster ignorieren" wird gemerkt** (`gui-settings.json panel_ignore_grid`), als Toolbar-Checkbox + im Größe-Dialog; **Drag/Resize/Dialog honorieren es** (`_snap_maybe`). **Größe-Bug gefixt:** in Frei-Modus 8-px-Untergrenze statt einer Rasterzelle → exakte Pixel bleiben erhalten. **Grid feiner** (1/16→1/24).
> - **Device-Explorer: Hinweis evdev vs. hidraw** (manche Geräte doppelt → hidraw für LEDs/Displays, sonst evdev) + Explorer-Button-Labels übersetzt.
> - **Manual §7** (Anordnen-Editor: Move/Resize/Entfernen/Umbenennen/Deko + LED-Bedingung als **Operatoren** statt ab/bis), **§4** (evdev/hidraw: manche doppelt). **Tests +4** (Deko/hidden/labels Round-Trip, clear wischt alles). **🔴 GUI visuell UNGEPRÜFT.**
> - **FULL i18n-SWEEP (Folge-Commit):** alle restlichen `tr()`-Literale in `gui.py`+`gui_mapper.py` übersetzt — **+234 `_STRINGS`-Einträge** (en/es/fr), generiert per Skript (`json.dumps`-Escaping, Assertion „jeder fehlende String abgedeckt"). Es bleiben nur **23 reine Symbole/Produktnamen/Fachtoken** (↺ ✓ 🪄 LED RPN SimVar min/max Encoder …) bewusst unübersetzt. i18n-Test grün (jeder Eintrag hat en/es/fr), ruff sauber.
>
> ## CUT 2026-08-23 (③) — LED-BEDINGUNG (an/aus/Fenster) + SWITCH-PANEL-GENERIC-LED-FIX + MAPPER-UX-BATCH, 468 Tests grün, ruff+mypy sauber (committet)
> **Mehrteiliger User-Auftrag rund um Mapper-Ausgaben + Bridge-GUI:**
> - **LED-Bedingung als Bereich statt nur `≥`-Schwelle** (User: „was, wenn ich UNTERschreiten muss? rote Gear-LED ist ein Fenster"): `GenericLed` bekommt **`off_at`** (obere Grenze) + `on_at` wird `float|None`; neue Methode `GenericLed.lit(value)` = `(on_at is None or v≥on_at) and (off_at is None or v<off_at)` → **an ab / an unter / Fenster** in EINEM Eintrag (nie mehr dieselbe LED doppelt; zwei Einträge auf einem Bit würden eh nur ge-ODER-t). `model_validator` weist leeres Fenster (off_at≤on_at) ab. Bestätigt: **Saitek-Gear rot = Fenster** `0<pos<0.95` (`mapping/leds.py gear_led_byte`, grün ab 0.95). `generic_panel.render` nutzt `led.lit`. Glow: `lamp_lit(value, on_at, off_at)` + `PanelElement.off_at` + `led_condition_text()` (≥/</Fenster-Label) verdrahtet in `_output_items`/`_lay_tiles`/`_draw_led`/`_live_tick`.
> - **GUI „+ Ausgabe → LED…" Dialog überarbeitet:** Felder **„Leuchtet ab (≥)" + „Leuchtet bis (<)"** (leer=keine Grenze) statt einzelner „Schwelle"; **Tooltips** an allen Feldern + **Hinweistexte** wie die Adresse (Byte/Bit/Offset) per „🔦 Adresse finden…" gefunden wird; Plausiprüfung (mind. eine Grenze, off>on). Zeilen-Layout auf laufenden Zähler umgebaut.
> - **BUG-FIX Switch-Panel:** eine per „+ Ausgabe" angelegte generische LED war im Nachbau UNSICHTBAR (Hand-Layout `_switch_panel`/`_radio_panel` zeichnete nur eigene Controls + gear_leds). Neu `panel_layout._extra_generic_elements(outs)` hängt generic_panel-LEDs/Displays in einem Streifen **unter** das Hand-Layout (y>1.0; Canvas wächst+scrollt), ref = echter Output-Index (via `_output_items` auf voller Liste gefiltert). Gilt für switch+radio.
> - **Geräte-Paket-Import ins Mapper-Rechtsklick-Menü** (neben Export — „gehört zusammen"); Toolbar-Button **entfernt** (User); Import bleibt auch bei Rechtsklick auf LEEREN Bereich erreichbar (`_dev_menu` zeigt dann nur Import).
> - **Umkehrfunktion `_pull_device`** „Mappings aus einem anderen Profil holen…" (Gegenstück zu „…übertragen…"): kopiert die Mappings des Geräts AUS einem gewählten Quellprofil INS aktuelle (nur Profile gelistet, die das Gerät wirklich mappen; Überschreib-Rückfrage; `copy_device_mappings` src→dst=current).
> - **„Alles starten"-Button** (Connection-Tab, `start_all`): Bridge starten → per `win.after` auf Port 7842 pollen (Bridge lauscht erst nach MSFS+Flug) → dann Mapper; Timeout ~10 s → Hinweis. i18n `conn.start_all`/`_no_port`. (Pendant zum bestehenden „Alles stoppen".)
> - **Bridge komplett aus der GUI (User: „ALLES aus der GUI, nicht nur manuell"):** der Installer (`setup-prefix.sh`) + udev + Start waren schon Buttons; **Lücke = Prefix FINDEN**. Neu `env_check.detect_prefixes()` (Python-Pendant zu `find-prefix.sh`: Standard-Roots + Flatpak + $STEAM_ROOT + flacher Scan von /media,/mnt,/run/media) + Connection-Button **„Suchen…"** (füllt Prefix-Feld, Auswahl bei mehreren). i18n `conn.detect*`.
> - **Anordnungsmodus: EIN Kontextmenü „Größe & Position…"** (statt nur „Größe in Pixel…"): Breite/Höhe/**X/Y** in px + Checkbox **„Raster ignorieren"** (aus=snap, an=pixelgenau). Position war vorher gar nicht editierbar.
> - **Manual:** Schritt 7 systematische Referenz **jeder Eingabe- (Button/Switch/Achse/Encoder/Hat/Selector) und Ausgabe-Art (LED/Display/Vorlage)** + LED-Bedingung (an/unter/Fenster mit Gear-Beispiel); Schritt 1 Anordnung um „Größe & Position" ergänzt; Schritt 9 Prefix **„Suchen…"**-Button.
> - **Tests +8 (460→468):** LED unter-Schwelle · LED-Fenster (Gear) · leeres Fenster abgelehnt · `lamp_lit` Fenster/unter · `led_condition_text` · generische LED unter Switch-Panel-Hand-Layout · `detect_prefixes` Standard-Roots + leer. ruff+mypy sauber, gui.py parst.
> **🔴 GUI visuell UNGEPRÜFT** (LED-Dialog-Felder+Tooltips · Switch-Panel-Extra-LED-Streifen · „Suchen…"-Button · Größe&Position-Dialog brauchen Display).
>
> ## CUT 2026-08-23 (②) — GERÄTE-PAKET (ein Gerät teilen) + KOMPLETTES HANDBUCH + DOKU-KONSOLIDIERUNG, 460 Tests grün, ruff+mypy sauber (uncommitted)
> **User: „profile für geräte importieren (knopf anordnungen + mapping)" → voller Umfang bestätigt (Anordnung + Mapping + Geräte-Def + Kalibrierung).** Neues **`device_package.py`** (Stil wie `backup.py`, injizierbare `root`/`user_dir`): ex-/importiert EIN Gerät als portable `.zip` = **DeviceDef** (`device.yaml`) + **Mapping** (`mapping.yaml`, bindings+outputs des Geräts aus EINEM Profil, kommentarerhaltend via `profile_writer.copy_device_mappings`) + **Panel-Layout** (`layout.yaml`, Knopf-Anordnung aus `panel-layouts.yaml`) + **Kalibrierung** (`calibration.yaml`) + Manifest (`kind: device-package`). Import registriert das Gerät ins **Overlay** (nie den versionierten Katalog; un-hided die id automatisch), stellt Layout+Kalibrierung wieder her, kopiert das Mapping ins gewählte **Zielprofil** (validate vor dump; Mapping übersprungen wenn kein Zielprofil). Neu: `profile_writer.loads()` (Gegenstück zu `dumps`). **GUI:** Mapper-Rechtsklick → „Als Geräte-Paket exportieren…"; Button „📥 Geräte-Paket importieren…" (liest Manifest, Confirm nennt Gerät+Zielprofil, dann reload). **CLI:** `export-device <id> <zip> --profile P` / `import-device <zip> [--profile P]`. **7 Tests** (`test_device_package.py`): 4-Teile-Export · Manifest · Round-Trip in frische Umgebung · Import un-hided · Import ohne Zielprofil überspringt Mapping · unknown device/profile · non-package. Real geprüft (CLI `export-device pedals --profile default`). **🔴 GUI visuell UNGEPRÜFT** (Rechtsklick-Export + Import-Button + Dialoge brauchen Display).
> **Dann Doku: User wollte EIN zusammenhängendes Handbuch mit ALLEM (Install-Skript, Prefix, evdev/hidraw, Profil-Transfer, GUI+Terminal-Wege, für Nicht-Programmierer) + „räum die Doku auf, prüf README-Links, halt es schlank" — und danach explizit: „Handbuch ENGLISCH wie die restliche User-Doku".** Gebaut **`docs/MANUAL.md`** (ENGLISCH, das eine End-to-End-Manual: Part A Schnellstart 4 Befehle → Part B Install → udev (GUI-Button + Terminal) → **evdev vs hidraw erklärt** → fremde HW registrieren/anlernen/deregistrieren → Mappen ohne Sim → **Profile: Transfer/Geräte-Paket/Backup** → Bridge/Prefix (Steam-Varianten) → Fliegen → Alltag/Troubleshooting/Datenorte; jeder Schritt 🖱 App + ⌨ Terminal, ✓-Checkpoints; Mapper-Buttons als DE-Label + EN-Gloss zitiert). **Konsolidiert:** `QUICKSTART.md` + `INSTALL.md` + `running.md` **GELÖSCHT** (voll aufgegangen), alle eingehenden Links umgebogen (README top + docs/README-Index neu, `install.sh`, `cli.py`-Docstring, `cheatsheet.md`, `geraete-baukasten-konzept.md`). `geraete-workflow.md` bleibt als Design/Status-Referenz (Concept-Sektion). Link+Anker-Checker: alle relativen MD-Links + In-Page-Anker grün.
> **Nächstes:** committen+pushen; GUI der beiden Features + Manual-Genauigkeit am Display gegenlesen.
>
> ## CUT 2026-08-23 — GERÄTE DEREGISTRIEREN (fremde User werden Edgars Geräte los), 453 Tests grün, ruff+mypy sauber (committet+gepusht auf `main`)
> **User-Problem: „alle Leute bekommen anfangs meine Geräte zu sehen".** Ursache: `config/devices.yaml` (versioniert, mitgeliefert) enthält Edgars Hardware (yoke/tq6/trim/pedals/Saitek-Panels); `load_device_catalog` merged nur das User-Overlay OBEN DRAUF — es gab keinen Weg, Katalog-Geräte wieder loszuwerden (Overlay kann nur ADD/OVERRIDE per id). Profile referenzieren diese ids, das versionierte YAML darf ein Fremder nicht editieren → **nicht-destruktive Hide-Schicht im User-Overlay** gebaut (parallel zu `show_gauges`/Overlay-Prinzip):
> - **`mapping/loader.py`**: neuer top-level Key **`hidden: [id,…]`** in `devices.local.yaml`. `load_device_catalog` filtert versteckte ids NACH dem Merge raus (robust gegen ein Overlay OHNE `devices`-Key → `model_validate({"devices": …or []})`). Neue reine Funktionen **`load_hidden_devices` / `hide_device` / `unhide_device`**; `hide_device` wirft eine Overlay-eigene Gerätekopie raus UND merkt die id in `hidden` (versioniertes Katalog-YAML unberührt, Profile bleiben). `add_device_overlay` **bewahrt** jetzt `hidden` und **entfernt** beim Re-Registrieren dieselbe id daraus (Re-Register = natürliches Undo). Da beide Listen (Mapper `dev_tree` via `build_device_rows` UND CLI) aus `load_device_catalog` kommen, wirkt der Filter überall.
> - **`gui.py`**: Mapper-Rechtsklick-Menü (`_dev_menu`) → **„Aus der Geräteliste entfernen…"** (`_deregister_device`, Confirm erklärt: nur für dich, Katalog/Profile bleiben, reversibel). Geräte-Explorer: **„Deregistrieren…"** (für verbundenes registriertes Gerät) + **„Ausgeblendete Geräte…"** (Listbox aus `load_hidden_devices` → „Wieder anzeigen“ = `unhide_device`). Wichtig: der Explorer zeigt nur VERBUNDENE Geräte, drum ist das Mapper-Kontextmenü der Hauptweg (dort sieht der Fremde auch Edgars NICHT-angesteckte Geräte).
> - **`cli.py`**: **`msfs-bridge deregister-device <id>`** (+ `--restore`, `--list`). End-to-end real geprüft (hide→`list-devices` lässt es weg, `--restore` holt zurück, Overlay wird zu `{}` aufgeräumt).
> - **Tests +7** (`test_device_overlay.py`): hide filtert Bundled + lässt `config/devices.yaml` unangetastet · hide wirft Overlay-Kopie raus · unhide stellt wieder her · add_device_overlay bewahrt `hidden` · Re-Register un-hided · load_hidden fehlende Datei. **453 grün, ruff+mypy clean, gui.py parst.**
> **🔴 GUI visuell UNGEPRÜFT** (Kontextmenü-Eintrag + Explorer-Buttons + „Ausgeblendete“-Dialog brauchen Display). **Offen/Nächstes:** GUI am Display sichten · Rest des Nordstern-Backlogs unten bleibt gültig. **Zur „Geräte-Profile importieren“-Idee (User):** die bestehende Export/Import (`backup.py`, Settings → „Sichern & Wiederherstellen“) deckt das schon ab — sie bündelt Profile (Mappings) + `panel-layouts.yaml` (Knopf-Anordnung) + `devices.local.yaml` (eigene Geräte). Nur eben als Komplett-Paket, NICHT pro Gerät. Ein fokussiertes „ein Gerät als teilbares Paket ex-/importieren“ (nur dessen DeviceDef + Mapping + Layout + Kalibrierung) wäre der Follow-up, falls gewünscht — User hat vorerst „reicht so, pushe“ signalisiert.
>
> ## CUT 2026-08-22 — REPO PUBLIC + CI ENDLICH GRÜN + ONBOARDING/DOKU + EXPORT/IMPORT + TAB-SICHTBARKEIT · nur noch `main` · 308 Tests grün
> **Große Aufräum-/Onboarding-Session. Repo ist jetzt ÖFFENTLICH** (github.com/creativeSloth/msfs-peripherals-bridge). **Alle Feature-Branches gelöscht (lokal+remote) — es gibt NUR NOCH `main`; künftige Arbeit direkt auf `main`** (oder neuem Branch). Die vorher „uncommitteten" Batches (u. a. MAPPER-UX A–F unten) sind dabei mit reingekommen und liegen jetzt auf `main`.
> **CI war SEIT MONATEN ROT → jetzt GRÜN.** Ursache war NICHT neuer Code: `ruff format --check` (Repo nie formatiert) + `mypy strict` (979 Fehler Alt-Bestand, davon 841 in `gui.py`). Behoben: **`ruff format`** übers ganze Repo (49 Dateien); **mypy 979 → 0** — `gui.py` (6800-Zeilen-Tkinter) per `[[tool.mypy.overrides]] ignore_errors=true` ausgenommen, `gui_mapper.py` bleibt geprüft (nur Annotationspflicht gelockert), ALLE Nicht-GUI-Module sauber annotiert + echte Typfehler behoben (evdev/hidraw-Reader-Rückgaben, engine None-Handling, radio bank-Union, outputs aux-Controller-Namenskollision, runtime StateDispatcher-`cast`, profile_writer/panel_layout u. a.). Overrides stehen in `pyproject.toml`.
> **Onboarding für Nicht-Techniker gebaut:** `docs/QUICKSTART.md` (Copy-paste, 4 Befehle) · **`install.sh`** (uv + venv + Pakete + udev in einem) · **`tools/find-prefix.sh`** (MSFS-Prefix über nativ/`.local`/Flatpak/2. Platte finden) · **`tools/install-udev-rules.sh`** · **GUI Connection-Tab „Geräte freischalten…"** = udev-Regeln via **`pkexec`** (grafischer Passwortdialog), `udev_setup.py` + Tests. `_spawn_and_tail` generalisiert (`log_name`/`running_key`).
> **Doku aufgeräumt + Englisch:** `docs/README.md` = aufgabenorientierter Index (Guides/Reference/Concept/Add-ons, Sprach-Marker 🇩🇪) · README **„👉 Start here"** ganz oben + Quick-start auf `install.sh` vereinheitlicht · **`docs/INSTALL.md` komplett auf Englisch** · **`docs/bridge-concept.md`** (kurzer Explainer, 2 Mermaid: Round-Trip + Session; aktuelles Protokoll inkl. `read_now`/`rpn`). `pyproject` Repository-URL korrigiert (Bindestrich).
> **NEU: Export/Import aller Nutzerdaten** (`backup.py`): bündelt **`profiles/` + `config/calibration.yaml` + das ganze `~/.config/msfs-peripherals-bridge/`** (Anordnung/eigene Geräte/Vorlagen/Einstellungen) in EINE `.zip` (Manifest `msfs-bridge-backup.json` + Path-Traversal-Schutz). CLI **`msfs-bridge export-config <zip>` / `import-config <zip>`**, GUI **Settings → „Sichern & Wiederherstellen"**. `tests/test_backup.py` (Round-Trip/Overwrite/Manifest/Non-Backup/Traversal). Real geprüft: sichert 7 Profile + Kalibrierung + 3 GUI-Dateien.
> **NEU: Tab-Sichtbarkeit** — Settings → **„Angezeigte Tabs" → „Instrumente-Tab (Gauges)"**, **DEFAULT AUS** (persistiert `show_gauges` in `gui-settings.json`, live umschaltbar via `nb.hide`/`nb.insert`). Helfer `load_show_gauges`/`save_show_gauges`.
> **🔑 WO LIEGEN NUTZERDATEN (User-Frage, wichtig):** IM REPO (geht bei frischem Clone verloren, wenn nicht committet): `profiles/*.yaml` (Mappings), `config/calibration.yaml`. IN `~/.config/msfs-peripherals-bridge/` (überlebt Neu-Clone): `panel-layouts.yaml` (Anordnung), `devices.local.yaml` (eigene Geräte), `output-templates.yaml`, `gui-settings.json`. → das Backup (oben) deckt BEIDES ab.
> **Verifiziert:** ruff/format/mypy(0)/pytest **308 grün**; GUI-4s-Start (DISPLAY=:0) sauber inkl. neuer Settings/Tab-Logik; CLI-Export real. **🔴 GUI visuell weiterhin nur kurz gesichtet** (Backup-Buttons + Tab-Toggle am Display ungeprüft; `pkexec`-Dialog einmal live ansehen).
> **Offen/Nächstes:** (1) `bridge/README.md`-Protokolltabelle auf `read_now`/`rpn` nachziehen (ist veraltet). (2) optional GitHub-Action-Versionen bumpen (Node-20-Deprecation-Warnung im CI-Lauf). (3) der ganze alte Mapper-Nordstern-Rest unten (Glow DME/ADF/XPDR, Saitek→`generic_panel`, GUI visuell prüfen) bleibt gültig.
>
> ## CUT 2026-08-19 — MAPPER-UX-BATCH A–F (uncommitted) + neues Gerät Logitech Extreme 3D Pro, 430 Tests grün, ruff clean
> **User steckte einen neuen Joystick an (Logitech Extreme 3D Pro, `046d:c215`, /dev/input/event23) und will ihn mappen.** Anleitung gegeben: im **Geräte-Explorer die evdev-Zeile** markieren → „Registrieren…" (Transport kommt aus der markierten Zeile!) → im Mapper `+ Eingabe ▾` je Knopf. Stick meldet: Achsen 0=X,1=Y,5=RZ(Twist),6=Throttle · Hat 16/17 · Buttons 288–299. Elevator-Trim am Hat = `ELEV_TRIM_UP`/`ELEV_TRIM_DN` (Impuls, kein SET).
> **Dann Mapper-Verbesserungs-Batch gebaut (uncommitted, User hat „committen" NICHT gesagt):**
> - **A · Anordnungsmodus Größe+Raster:** Raster gröber `_PANEL_GRID 1/24→1/16` + jetzt auch **horizontale** Linien. **Resize per Maus** (blaue Eck-Anfasser `pgrip:{n}`, Gummiband, Snap) + **Größe in Pixel** (Rechtsklick im Edit-Modus → `_panel_size_dialog`, exakt OHNE Snap). Persistenz erweitert: `save_panel_layout_override(dev,key,x,y,w=None,h=None)` speichert `[x,y]` ODER `[x,y,w,h]`, reiner Move bewahrt vorhandene Größe; `load_panel_layout`→2- od. 4-Tupel; `panel_layout.apply_layout_overrides` setzt x/y (+ w/h bei len≥4). Rückwärtskompatibel.
> - **B · Deadzone als ROH-Fenster (Laufzeit!):** User will KEINE implizite Umrechnung — Deadzone = echtes Eingangs-**min/max** mit Plausiprüfung. `Transform.deadzone_min/max: int|None` (additiv, alte `deadzone`-Fraktion bleibt), `transforms.normalise_window()` (weich reskaliert: innen 0, außen stufenlos bis ±1), `shape_axis` nimmt Fenster wenn beide gesetzt (Vorrang vor Fraktion). Editor: dz-Feld → **„Totzone (roh) min/max"**; `_form_transform` parst Fenster + prüft `min<max` und `innerhalb Eingang min…max` (ValueError→inline). `_transform_fields` dumpt `dz_min/dz_max` (+ `tf_deadzone` als versteckten Legacy-Passthrough).
> - **C · Achsen-Ausgang default −16383…16383:** `_axis_out_defaults()` setzt bei NEUEN Achsen-Bindings out_min/out_max (in `_new_binding` + `_panel_map_empty`).
> - **D · Duplizieren `_1`-Suffix:** `gui_mapper.next_copy_name(name, existing)` (strippt trailing `_N`, nächste freie `_N`) statt „ (Kopie)"; in `_ed_duplicate` verdrahtet.
> - **E · Aktions-Editor kind-abhängig + Wording „Schritt"→„Event" (sichere Neurahmung, KEIN Modell/Laufzeit-Eingriff):** `_seq_render` jetzt **kind-abhängig** — **Taster** = EINE Liste „Events (beim Drücken)" (nur on_edge); **Schalter** = „beim Einschalten"/„beim Ausschalten" (on/off). Off-Liste bleibt auch sichtbar, wenn ein geladenes Binding schon off-Events hat (kein Datenverlust). „Mehrschritt"→„Mehrere Events", „+ Schritt"→„+ Event", `_TYPE_NOTE`/Notizen/gui_mapper-Fehlermeldungen entschärft. `_ed_show_fields` ruft `_seq_render()` beim Anzeigen (reagiert auf Arten-Wechsel). **User-Entscheid: Schalter-Standardfall = Einzel-Event OHNE Wert = stateful 1/0 (Option b), Ein/Aus-Listen nur für Sonderfall.**
> - **F · Geräte-Mappings in anderes Profil übertragen:** `profile_writer.copy_device_mappings(src,dst,device_id,overwrite)` (deep-copy der YAML-Nodes → Kommentare/Flow-Style bleiben, Doks unabhängig) + `device_mapping_counts`/`_device_block`. GUI: **Rechtsklick auf Gerät in dev_tree** (`_dev_menu`) → „Mappings in anderes Profil übertragen…" → `_transfer_device`-Dialog (Ziel-Profil-Liste, Überschreib-`askyesno` wenn Ziel das Gerät schon mappt, `validate` vor `dump`).
> - **Tests +11 (419→430):** normalise_window/shape_axis-Fenster+Vorrang · deadzone-Fenster Round-Trip+Plausibilität · next_copy_name · apply_layout_overrides 4-Tupel · Persistenz w/h+Move-bewahrt-Größe · copy_device_mappings transfer/deepcopy-Unabhängigkeit/overwrite-Flag. (E: 1 Fehlermeldungs-Match angepasst.)
> **🔴 GUI visuell UNGEPRÜFT** (Canvas-Resize/px-Dialog · Deadzone-Felder · Achsen-Defaults · Duplizier-Name · kind-abhängiger Event-Editor · Geräte-Rechtsklick→Transfer — alles braucht Display).
> **🟡 BACKLOG (dokumentiert, bewusst NICHT gebaut):**
> - **E-tief · „Event(s)"-Liste ERSETZT den Einzel-Picker ganz (kein „Mehrere Events"-Häkchen mehr):** die Liste als EINZIGE Mechanik, 1 Eintrag = Normalfall, automatisches Einklappen eines 1-Eintrags auf eine *plain* Action (statt Sequence) für sauberes YAML. **Blocker:** `WriteStep` kann heute nur `event`/`simvar` — NICHT `event_from_var`/`rpn`; eine echte Voll-Vereinheitlichung braucht WriteStep-Erweiterung ODER den Picker als Sonderpfad für efv/rpn. Eigener, VISUELL geprüfter Durchgang (berührt `form_to_binding`/`_form_action`/`_ed_apply`-Glue). Die jetzige Neurahmung (E oben) liefert Wording + kind-Framing schon ohne dieses Risiko.
>
> ## CUT 2026-08-12 — GLOW-SCHEIBE 2: controller-treue Segment-Formatierung + generic_panel-LEDs/Displays glühen, 419 Tests grün, ruff clean
> **User: „weiter machen" → „weiterbauen".** Erst die zwei vorigen Batches (① Glow-aus-Sim + ② Encoder-als-Eingabetyp) auf User-Wunsch **committet: `511ccef`** (Branch `feat/mapper-panel-nachbau`, 1 vor origin — **noch NICHT gepusht**). Dann die nächste Glow-Scheibe gebaut (uncommitted; User hat „committen" für diese hier NICHT gesagt):
> - **`panel_layout.py`**: `PanelElement.fmt: str` (controller-treues Segment-Format) + reiner Helfer **`format_segment(value, fmt)`** — `""`=roh `.4g` (wie `_fmt_value`), `"int"`=gerundet (wie `format_row`), `"dec:N"`=N Nachkommastellen, negativ/None→blank (wie `format_measure`/`format_frequency`). Verdrahtet: **Radio-freq Act/Stby → `dec:2`** (NNN.NN, Ruhe-/Grob-Ansicht) · **Multi-Selektor-Zellen → `int`** (Ganzzahl, `format_row`) · **generic_panel jetzt im Nachbau**: `_output_items` neue Zweige `leds`/`displays` → LED-Element (var+on_at, glüht wie Fahrwerks-LED) + 7-Seg-Segment (var + fmt `dec:{decimals}` bzw. `int`). `_output_items`-Tupel auf **9** erweitert (fmt), `_lay_tiles` entpackt mit.
> - **`gui.py`**: `_draw_segment` registriert `fmt` im sim-Handle; `_live_tick` malt Segmente jetzt via `panel_layout.format_segment(val, fmt)` statt rohem `_fmt_value`. generic-LEDs glühen automatisch über den bestehenden `_draw_led`-Lamp-Pfad (tragen jetzt var+on_at).
> - **Tests (+2, 417→419):** `format_segment` (int/dec:N/negativ-blank/roh/bool/non-numeric) · generic_panel-LED+Displays glühen (var/on_at/fmt) · freq-Segmente `dec:2` + Multi-Selektor `int` in bestehende Tests ergänzt. ruff clean, gui.py parst.
> **🔴 GUI visuell UNGEPRÜFT** (Canvas + laufende Bridge): freq zeigt jetzt 118.00 statt „118"? eigenes generic_panel-Gerät → LEDs grün + Zahlen? **Bewusst OFFEN (Rest der Glow-Nordstern-Punkt-1):** (a) **DME/ADF/XPDR** noch NICHT verglüht — brauchen mehr als 1-Var-pro-Element: DME-Quelle hängt an `source_var` (welcher NAV?), ADF/XPDR sind per-Ziffer mit Cursor/Punkt (`_render_adf/_render_xpdr`). (b) **Multi-`bool_leds`-Knopflichter** (BUTTON_LIGHT) + `ap_master`/`mode_leds` glühen noch nicht (Multi-Panel-Knöpfe sind Bindings, die Lichter separate Lampen). (c) generic_panel-LED-Position ist Auto-Grid. (d) Stale-Wert nach Disconnect bleibt (kosmetisch).
>
> ## CUT 2026-08-11 (②) — ENCODER als Eingabetyp im Mapper (2-Richtungs-Anlernen), 417 Tests grün, ruff clean [COMMITTET 511ccef]
> **User-Frage: „müssten Encoder nicht wie ein Eingabeknopf hinzufügbar sein + Kontextmenü entsprechend?"** Analyse ergab: Encoder ist im **Struktur-Scanner** (`_open_input_scan`, `InputBlock.kind=encoder`) längst erstklassig, im **Mapper (Bindings)** aber NICHT — `SourceKind` = nur axis/button/hat/switch, ein Encoder = **zwei Button-Bindings** (cw/ccw je Event). **User-Entscheid (Rückfragen):** (a) Modell = 2 Button-Bindings (kein Engine-Eingriff). (b) Oberflächen = `+ Eingabe` **und** Kontextmenü — ABER User präzisierte: **Swap/Switch sind KEINE Lücke** (Swap = normaler Taster, Switch = schon eigener Kind); und für die **Saitek-Panels (switch/multi/radio) KEIN Customizing** — die sind statisch, haben eigene Controller, Encoder/Swap/Selektor kommen aus der Vorlage (Kontextmenü öffnet dort eh „Bearbeiten…"). Generische Geräte haben zudem gar keine leeren Platzhalter → das Kontextmenü blieb bewusst unangetastet; der sinnvolle Ort ist NUR `+ Eingabe`.
> **Gebaut (uncommitted; User hat „committen" NICHT gesagt):**
> - **`panel_layout.is_static_panel(profile, device_id)`** (rein, getestet): True für Hand-Layouts (switch/radio) + Geräte mit `gear_leds`/`multi_panel`/`radio_panel`-Output. Steuert, wo die generische Encoder-Aktion erscheint.
> - **`gui.py`**: `+ Eingabe`-**Knopf → Menubutton `+ Eingabe ▾`** mit `[Taster/Schalter/Achse/Hat]` (öffnet Editor mit vorgewähltem Kind — `_new_binding(kind)`) **+ „Encoder (2 Richtungen)…"** — Menü per `postcommand` neu gebaut, Encoder-Eintrag NUR wenn `not _is_static_panel(dev)`. **`_add_encoder_binding`**: 2-Schritt-Flanken-Capture (`_capture_codes`, hidraw `edge_count_reader` / evdev `button_edge_reader` + `winning_code`, wie `_learn_code`) → Name-Abfrage → öffnet Binding-Editor für **CW** vorbefüllt (button+code+Name+event); beim Speichern **Continuation-Hook `etgt["then"]`** → öffnet direkt den **CCW**-Editor. Hook sauber: `_open_editor`/`_ed_close` räumen `then` (kein Leak bei Abbruch), `_ed_apply` fängt `nxt` früh ab und feuert nach erfolgreichem Save.
> - **Tests (+1, 416→417):** `is_static_panel` (Saitek True, generische Geräte/None False). Capture/Editor-Chain = GUI-Glue, headless nicht sinnvoll testbar.
> **🔴 GUI visuell UNGEPRÜFT** (braucht Display + Gerät mit Encoder): Yoke/TQ6/fremdes Panel wählen → `+ Eingabe ▾ → Encoder…` → CW/CCW drehen → Name → CW-Event (…_INC) → CCW-Event (…_DEC)? Bei Saitek fehlt der Encoder-Eintrag (gewollt). **Rand:** bricht der User den CW-Editor ab, entsteht kein Binding; speichert er CW aber bricht CCW ab, existiert nur eine Richtung (nachträglich ergänzbar).
>
> ## CUT 2026-08-11 — NORDSTERN-REST ①: GLOW-AUS-SIM (LEDs/Segmente live), 416 Tests grün, ruff clean
> **User-Auftrag „mach weiter".** Der Nordstern-Rest-Punkt (1) gebaut — die Anzeige-Elemente im Nachbau zeigen jetzt LIVE den Sim-Wert (uncommitted bis nächster Commit; **User hat „committen" NICHT gesagt** → liegt bereit):
> - **`panel_layout.py`**: `PanelElement` bekommt `var: str|None` (treibende Sim-Var) + `on_at: float|None` (Lampen-Schwelle; None = numerisches Segment). Reiner Helfer **`lamp_lit(value, on_at)`** (None→dunkel, bool as-is, Zahl ≥ Schwelle, non-numeric truthy→an). Befüllt an den Bau-Stellen: **Fahrwerks-LEDs** (switch_panel, `getattr(GearLedOutput, nose/left/right)`, on_at 0.5) · **Multi-Selektor-Segmente** (`selector[i].simvar`, numerisch) · **Radio Act/Stby** NUR bei `freq`-Banks (`bank.active/standby`); dme/adf/xpdr brauchen Ziffern-Formatierung → später. Helfer `_display_var(o,p)`; `_output_items`-Tupel auf 8 erweitert (var/on_at), `_lay_tiles` entpackt mit.
> - **`gui.py`**: neues `pcanvas["sim"]` (Var→Zeichen-Handles), in `_render_panel_canvas` geleert; `_draw_led`/`_draw_segment` registrieren ihre Handles (LED: oval+off-Farben+on_at; Segment: text+Fallback-Label). Neuer **`panel_glow`-Hook** (Muster wie `gauge_hook`) → in `_resubscribe` eingehängt: der Monitor abonniert genau die gezeichneten Display-Vars, aber NUR wenn Mapper-Tab + Panel-Ansicht sichtbar. `_render_panel_canvas` ruft am Ende `_resubscribe()`. Der bestehende **`_live_tick` (100 ms)** malt jetzt zusätzlich aus `monitor.values()`: LED-Glow (grün `_LAMP_ON` via `lamp_lit`), Segment-Text (`_fmt_value`) — **unabhängig vom physischen Gerät** (funktioniert auch ohne angestecktes Panel; nur die laufende Bridge liefert Werte).
> - **Tests (+3, 413→416):** Gear-LEDs tragen ihre Var+Schwelle · `lamp_lit`-Schwellen/Typen · Radio-freq Act/Stby-Var · (Multi-Selektor-Var-Assertion in bestehenden Test ergänzt). ruff clean, Import ok.
> **🔴 GUI visuell UNGEPRÜFT** (Canvas + laufende Bridge nötig): GUI→Mapper→Gerät mit Anzeigen (switch_panel/multi/radio) wählen, Bridge fliegen lassen → leuchten die Fahrwerks-LEDs grün / zeigen COM/NAV-Segmente die Frequenz? **Bewusst OFFEN (dokumentiert):** (a) Segment-Formatierung ist roh (`_fmt_value` .4g) — freq in Hz sähe krude aus; controller-treue Formatierung = Follow-up. (b) DME/ADF/XPDR-Segmente + DOT + Multi-`bool_leds`-Button-Lichter + generic_panel-LEDs sind noch NICHT als Glow-Elemente verdrahtet (bool_leds/generic-LEDs werden aktuell gar nicht als Elemente gezeichnet). (c) Stale-Wert nach Bridge-Disconnect bleibt stehen (Monitor prunt erst bei Namenswechsel) — kosmetisch.
> **🔴 NORDSTERN-REST (aktualisiert):** (1) Glow-aus-Sim ✅ erste Scheibe (Lampen bool + freq/selector numerisch) — Rest = Formatierung + weitere Element-Typen (s. o.). (2) Saitek-Panels auf `generic_panel` heben (Endspiel, laufzeit-riskant, eigener Durchgang). (3) `detail`-Treeview ganz aus dem Layout (kosmetisch).
>
...
> - **Sicherung** `backups/2026-08-07/` (alle Profile, config/, Overlay force-added, gui-settings) + **`tools/dump-device-functions.py`** (liest Profil+Katalog → Markdown-Report je Gerät: Bindings/Anzeigen/atomare `template_elements` = Neuaufbau-Checkliste; erzeugt `device-functions.md`) + **`tools/simulate-from-scratch.sh`** (Sandbox via `MSFS_BRIDGE_HOME`+`XDG_CONFIG_HOME` auf Wegwerf-Dir mit leerem Katalog/Profil → „bei null neu erkennen" GEFAHRLOS durchspielen; echte Dateien bleiben zu). **Entwarnung:** Profile sind alle in git eingecheckt = ohnehin wiederherstellbar; Baukasten schreibt nur ins Overlay, nie in Profile.
> - **Mapper-Knopfleiste umstrukturiert** (gui.py ~2170): früher `[+ Saitek-Panel ▾][+ Binding][Duplizieren][Entfernen]` → jetzt **zwei Gruppen mit Trenner**: `[+ Eingang][Vorlage ▾]  |  [Duplizieren][Entfernen]`. „+ Binding"→**„+ Eingang"** (klar = Input). „+ Saitek-Panel"→**„Vorlage ▾"**: erkennbar „ganzes Panel = Knöpfe+Anzeigen in einem Rutsch"; Menü = eingebaute Saitek-Vorlagen + **eigene Vorlagen** (Untermenü Anlegen/Löschen) + **„Aktuelles Gerät als Vorlage speichern…"**. Persistenz: `config.output_templates_file()` (`~/.config/.../output-templates.yaml`) + `loader.load/save/delete_output_template`. 1 Test. **⚠️ Vorlagen bleiben Saitek-Panel-Blöcke (hidraw-only); generisches „+ Anzeige einzeln mappen" für Fremdgeräte = Schritt E.**
> **③ evdev-Button/Switch/Encoder-Capture:** reines **`evdev_reader.key_edges(last, events)`** (zählt 0→1-Press-Flanken je Keycode, ignoriert Halten/Autorepeat/Release — Pendant zu `hidraw_reader.count_rising_edges`) + evdev **`button_edge_reader(path)`** (`(read,close)`, draint EV_KEY nicht-blockierend, kumuliert Flanken; Gewinner via `hidraw_reader.winning_code`). **GUI:** `_capture` ist jetzt **reader-agnostisch** (`opener`-Param, Default = hidraw `edge_count_reader`); `_add_input` routet bei `transport==evdev` Button/Schalter/Encoder über `button_edge_reader`. Damit ist der Wizard für FRISCHE evdev-Geräte komplett (Achsen ②, Buttons/Switch/Encoder ③). 4 Tests. **⚠️ Rest-Lücke:** ein evdev-Encoder, der als REL/Achse statt als Key-Events meldet, wird von `button_edge_reader` nicht erkannt (Saitek/Yoke melden Buttons als Keys → passt; exotische Encoder = Follow-up).
> **① Schritt A:** reine **`gui_mapper.template_elements(ddef, profile) → (list[InputBlock], list[OutputBlock])`** — projiziert das HEUTIGE hartverdrahtete Mapping eines Geräts in atomare Bausteine: jedes Plain-Binding → 1 `InputBlock` (kind+code+Achsen-Kalibrierung aus `source`; **Hat → `selector`** über seine Richtungs-Codes); jeder Panel-Controller-Output → seine Input-CONTROLS als `InputBlock` (Wert-/Ring-Encoder, Modus-Wahl-Selektor, SWAP, Dimmer) **plus** physische WRITE-Atome als `OutputBlock` (Multi: 8 Knopf-LEDs + 1 Display cells=10; Radio je Unit: außen/innen-Encoder + SWAP + Modus-Selektor, dazu Aktiv+Standby-Display je 5 Zellen; Switch: 3 Fahrwerks-LEDs). **GUI:** im Elemente-Editor (`gui.py _open_input_scan`) Button **„Aus Vorlage füllen…"** → `_fill_from_template` merged NUR fehlende Blöcke (dedup per Name), Confirm-Dialog, `_persist`+`_refill`. 4 headless-Tests.
> **② evdev-Achsen-Live-Capture:** reine **`evdev_reader.winning_axis(spans, min_span=8)`** (Pendant zu `hidraw_reader.winning_code`: breiteste beobachtete Achsen-Spanne gewinnt; Hat-±1 wird per Schwelle gefiltert) + 4 Tests (`tests/test_evdev_reader.py`). **GUI:** `_open_input_scan` holt jetzt auch einen `evdev_path`; „Achse" im Input-Menü ruft neues **`_capture_axis`** (live `live_state_reader`, akkumuliert je ABS-Kanal min/max, zeigt „erkannt: Achse N [lo..hi]", „Fertig" → `InputBlock(kind=axis, code, raw_min, raw_max)`). Damit ist Yoke/Pedale/TQ6-Achsenscan komplett (vorher nur Meldung „folgt").
> **🔴 SOFORT nächste Session:** (1) **VISUELL prüfen** (Display+HW): Mapper-Knopfleiste neu (`+ Eingang` / `Vorlage ▾` mit „als Vorlage speichern") + Elemente-Editor „Aus Vorlage füllen…" + „+ Input anlernen…" (Achse/Taster/Schalter an Yoke/Pedale). (2) **SCHRITT E — der Nordstern-Rest, jetzt dran** (User: „mach weiter mit dem Nordstern"). **Architektur-Seam gefunden:** `outputs.OutputManager._controllers: dict[device_id, PanelController]` (Protocol in `outputs.py:66`) + `_gear`; heute aus `profile.outputs` (typisierte Blöcke MultiPanelController/RadioPanelController) gebaut. **Plan E:** einen **generischen Controller** bauen, der `ddef.inputs`/`ddef.outputs` (atomare Blöcke) + eine Mapping-Schicht konsumiert und dieselbe `PanelController`-Schnittstelle erfüllt (`consumes/on_event/on_state/subscriptions/refresh_after`), dann in `OutputManager` neben die hardcodierten stellen → schrittweise ablösen. **⚠️ Berührt die LIVE-Laufzeit (fliegt das Flugzeug) → eigener, sorgfältig getesteter Durchgang, nicht nebenbei.** Vorstufe ohne Runtime-Risiko: reine „atomare Elemente + Mapping → Engine-Bindings/Reports"-Funktion, headless testbar, bevor der Manager umgestellt wird.
> **⚠️ TRANSITIONS-HINWEIS (bewusst so gelassen):** solange die Templates die Laufzeit treiben, zählt die Geräteübersicht geseedete Blöcke ZUSÄTZLICH zu den Template-Atomen (leichte Doppelzählung in `atomic_{in,out}put_count`). Löst sich mit **Schritt E** auf, der die Templates durch die generische Laufzeit über `ddef.inputs/outputs` ERSETZT (= echte Parität, großer Brocken, jetzt der Nordstern-Rest).
>
