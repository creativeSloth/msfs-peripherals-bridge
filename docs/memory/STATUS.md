...
> **🔴 TODO nächste Session:** (1) Session-Änderungen committen (wenn User will) — Doku + Monitor-Rename + **Baukasten-Schritt A** (inventory/overlay/CLI/GUI-Explorer). (2) **Geräte-Explorer visuell am Display prüfen** (Button „🔍 Geräte-Explorer…" im Mapper → Dialog → fremdes Gerät registrieren → taucht es in der Geräteliste auf?). (3) **Schritt B starten:** Input-Scan + **Alias**-Mapping (Kontextmenü am Gerät, `edge_count_reader`) + Kalibrierung fürs selbst-angelegte Gerät. (4) Monitor-Sim-Prüf-Konzept. (5) Nachbau visuell/HW sichten (braucht Display+Panel).
> ---
