...
> - **Manual §7** (Anordnen-Editor: Move/Resize/Entfernen/Umbenennen/Deko + LED-Bedingung als **Operatoren** statt ab/bis), **§4** (evdev/hidraw: manche doppelt). **Tests +4** (Deko/hidden/labels Round-Trip, clear wischt alles). **🔴 GUI visuell UNGEPRÜFT.** **⚠ Offen:** noch **244/457 `tr()`-Strings in gui.py unübersetzt** (voller i18n-Sweep = eigener Durchgang).
>
